...
def _attn_fwd_inner(
	...
	if BIAS_ON:
		if PADDED_COLS:
			bias = tl.load(
				...
				& ((index_start_n + offs_n) < actual_seqlen_k)[None, :],
				other=0.0,
			)
		else:
			bias = tl.load(
	...
	if MASKED and IS_CAUSAL:
		causal_mask = (
			offs_m[:, None]
			>= (index_start_n + offs_n - actual_seqlen_k + actual_seqlen_q)[None, :]
		)
...
def _fwd_attention_kernel_call(
	...
	if varlen_mode:
		...
		cum_seqlens_q = cum_seqlens_q.at[1:].set(
			jnp.cumsum(attention_mask.sum(axis=1, dtype="i4"), axis=0, dtype="i4")
		)
