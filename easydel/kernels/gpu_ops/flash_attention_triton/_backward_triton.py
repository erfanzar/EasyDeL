...
def _attn_bwd_block_dkdv(
	...
	index_begin_m = (
		max(index_start_n + actual_seqlen_q - actual_seqlen_k, 0) if IS_CAUSAL else 0
	)
	...
	if index_next_start_n < index_end_n:
		for index_start_n in range(index_next_start_n, index_end_n, BLOCK_N):
			pad_cols = (not EVEN_N) or (
				VARLEN and (index_start_n + BLOCK_N > actual_seqlen_k)
			)
			dq = _attn_bwd_dq(
				index_start_n,
				q,
...
def _bwd_attention_kernel_call(
	...
	attention_mask: tp.Optional[chex.Array],
	o: chex.Array,
	M: chex.Array,
	dropout_prob: float,
	causal: bool,
	softmax_scale: tp.Optional[float],
	dropout_seed: tp.Optional[int],
	varlen_mode: bool,
):
