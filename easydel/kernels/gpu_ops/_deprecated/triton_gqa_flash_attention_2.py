...
def _fwd_attention_kernel(
    ...
    O_Block_ptr = tl.make_block_ptr(
        base=Outp + (off_b * stride_ob + off_h * stride_oh + off_gp * stride_og),
        shape=(seqlen_q, headdim),
        block_shape=(BLOCK_M, BLOCK_HEADDIM),
        strides=(stride_om, 1),
        offsets=(start_m * BLOCK_M, 0),
...
def _bwd_do_attention_kernel(
    ...
    o_ptrs = (
        Outp + off_b * stride_ob + off_h * stride_oh + off_gp * stride_og + offs_m[:, None] * stride_om + offs_d[None, :]
    )
    do_ptrs = (
        Do
        + off_b * stride_dob
...
def _bwd_attention_kernel_call(
    ...
    """
    (o, l_se, query, key, value, bias) = residual
    ...
    if FLASH_ATTN_BWD_:
        ...
        # BATCH  , HEADS    , _
        stride_lb, stride_lh, stride_lg, _ = get_strides(l_se.shape)
