...
class RunTimeConfig:
    ...
    refrence_model_repo_id: str | None = field(
        default=None,
        metadata={"help": "The repository ID for the reference model. If None, defaults to repo_id."},
    )
    ...
    def __post_init__(self):
        ...
        if self.refrence_model_repo_id is None:
            self.refrence_model_repo_id = self.repo_id
        if isinstance(self.sharding_axis, str):
            self.sharding_axis = tuple(map(int, self.sharding_axis.split(",")))


parser = ed.utils.DataClassArgumentParser((ed.DPOConfig, RunTimeConfig))
dpo_config, runtime_config = parser.parse_args_into_dataclasses()

runtime_config: RunTimeConfig
dpo_config: ed.DPOConfig
...
def main():
    ...
    load_kwrags = dict(
        ...
        config_kwargs=ed.EasyDeLBaseConfigDict(
            freq_max_position_embeddings=dpo_config.max_length,
            mask_max_position_embeddings=dpo_config.max_length,
