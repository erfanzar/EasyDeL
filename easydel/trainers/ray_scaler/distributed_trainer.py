...
class RayDistributedTrainer:
	...
	_processor_loader_class: tp.Type[PreTrainedTokenizer] = AutoTokenizer
	...
	def __init__(
		...
		if model_task is None or model_type is None:
			...
			assert model_class is not None, (
				"model_class must be provided if model_task and model_type are not."
			)
		...
		if model_class is None:
			...
			assert model_class_retrieved is not None, (
				f"Could not retrieve model class for {model_type} and {model_task}"
			)
			self.model_class = model_class_retrieved
		else:
			self.model_class = model_class

		self.config_scaling_variables = (
			RayDistributedTrainer.CONFIG_SCALING_VARIABLES.copy()
		)
	...
	@staticmethod
	def extract_column_names(dataset: Dataset) -> tp.Optional[tp.List[str]]:
	...
	def create_config(self, scaling_index: int) -> EasyDeLBaseConfig:
		...
		current_scaling_variables = {
			k: v * scaling_index
			for k, v in copy.deepcopy(self.config_scaling_variables).items()
		}
