...
def training_step(
	...
	def loss_fn(tree, minibatch):
		...
		if "margin" in minibatch:
			loss = -jax.numpy.mean(
				jax.nn.log_sigmoid(rewards_chosen - rewards_rejected - minibatch["margin"])
			)
		else:
			loss = -jax.numpy.mean(jax.nn.log_sigmoid(rewards_chosen - rewards_rejected))

		if center_rewards_coefficient is not None:
			loss += center_rewards_coefficient * jax.numpy.mean(
...
def evaluation_step(
	...
	*_, partition_spec = make_assertions_and_get_sizes(
	...
	def loss_fn(tree):
		...
		if "margin" in batch:
			loss = -jax.numpy.mean(
				jax.nn.log_sigmoid(rewards_chosen - rewards_rejected - batch["margin"])
			)
		else:
			loss = -jax.numpy.mean(jax.nn.log_sigmoid(rewards_chosen - rewards_rejected))
