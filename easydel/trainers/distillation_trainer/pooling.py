...
"""Utilities for feature shape matching via pooling for distillation.

This module provides pooling functions to resize feature tensors between
teacher and student models with different architectures or dimensions.
"""
...
class PaddingMode(enum.Enum):
	"""Feature shape padding modes for pooling operations."""
...
def avg_pool_array_to_target_shape(
	...
	"""Reduces a JAX array to a target shape using average pooling.
	...
	to achieve the specified target shape. Useful for matching feature
	dimensions between teacher and student models in distillation.

	Args:
		input_array: The input JAX numpy array to pool.
	...
	Example:
		>>> teacher_features = jnp.ones((32, 512, 4096))  # [batch, seq, hidden]
		>>> student_features = jnp.ones((32, 512, 2048))
		>>> pooled = avg_pool_array_to_target_shape(
		...     teacher_features, student_features.shape
		... )
		>>> pooled.shape
		(32, 512, 2048)
	"""
	...
	if input_rank != len(target_shape):
		raise ValueError(
			f"Input array rank ({input_rank}) must match target_shape rank ({len(target_shape)})."
		)
	...
	# spatial: batch=1, spatial=(D1,...,Dn), channels=1 for the avg_pool function
