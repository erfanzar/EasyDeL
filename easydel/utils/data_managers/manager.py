...
class DataManager:
	...
	def _process_text_dataset(
		...
		def transform_fn(example):
			try:
				...
				raise KeyError(
					f"couldn't access {inform.content_field} Available Keys {example.keys()}."
				) from e
	...
	def load_from_config(
		cls,
		config_path: tp.Union[str, Path],
	) -> tp.Tuple[DatasetMixture, tp.Any]:
		"""
		...
		for ds_config in config_dict.get("informs", []):
			if "pixel_field" in ds_config:
				inform = VisualDatasetInform(
					...
					image_size=tuple(ds_config["image_size"])
					if "image_size" in ds_config
					else None,
				)
			else:
				inform = TextDatasetInform(
