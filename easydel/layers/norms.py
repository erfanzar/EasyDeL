...
from easydel.infra.utils import ACT2FN, ACT2FN_TYPES
...
class PlusRMSNorm(nn.Module):
    ...
    def __call__(self, x: Float[Array, "... dim"]) -> Float[Array, "... dim"]:
        ...
        if self.param_dtype in float8s or self.dtype in float8s:
            ...
            x = x.astype(jnp.promote_types(self.dtype, jnp.float32))
        output = self._norm(x).astype(self.dtype)
        weight = self.kernel.astype(self.dtype) + self.plus
        return (weight * output).astype(org_dtype)


class GatedRMSNorm(nn.Module):
    """Root Mean Square normalization layer (Gated).
    ...
    def __init__(
        ...
        dtype: jnp.dtype = jnp.bfloat16,
        param_dtype: jnp.dtype = jnp.bfloat16,
        gate_type: ACT2FN_TYPES = "silu",
        *,
        rngs: nn.Rngs | None = None,
    ) -> None:
        """Initialize GatedRMSNorm layer.
    ...
    @jax.named_scope("easydel-gated-rmsnorm")
    def __call__(self, x: Float[Array, "... dim"], gate: Float[Array, "..."]) -> Float[Array, "... dim"]:
        """Apply RMS normalization to input.
        ...
        if self.param_dtype in float8s or self.dtype in float8s:
            ...
            x = x.astype(jnp.promote_types(self.dtype, jnp.float32))
        output = self._norm(x).astype(self.dtype)
        output = (self.kernel.astype(self.dtype) * output) * ACT2FN[self.gate_type](gate.astype(self.dtype))
