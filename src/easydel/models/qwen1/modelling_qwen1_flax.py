        ...
        if rotary_pos_emb_list is not None:
            ...
            if len(rotary_pos_emb_list) == 1:
                ...
                for i, rotary_pos_emb in enumerate(rotary_pos_emb_list):
                    rotary_pos_emb = [
                        i[:, -current_length:, :, :] for i in rotary_pos_emb
                    ]
                    rotary_pos_emb = (rotary_pos_emb,) * 2
                    q_pos_emb, k_pos_emb = rotary_pos_emb
                    query_list += [
                        apply_rotary_pos_emb(
                            query[i : i + 1, :, :], q_pos_emb[1], q_pos_emb[0]
                        )
                    ]
                    key_list += [
                        apply_rotary_pos_emb(
                            key[i : i + 1, :, :], k_pos_emb[1], k_pos_emb[0]
                        )
                    ]
    ...
    def setup(self) -> None:
        ...
        if self.config.gradient_checkpointing != "":
            attn_block = nn_partitioning.remat(
                FlaxQwen1Attention,
                static_argnums=(1, 3, 4, 6, 7, 8, 9, 10, 11),
    ...
    def __call__(
        ...
        if self.config.use_scan_mlp:
            ...
            feed_forward_hidden_states = einops.rearrange(
                feed_forward_hidden_states, "... b s d -> ... (b s) d"
            )
        else:
...
class FlaxQwen1PreTrainedModel(BaseNNXModule):
