...
class FlaxMambaMixer(nn.Module):
    ...
    def setup(self) -> None:
        ...
        if self.config.time_step_init_scheme == "constant":
            init_kernel_dt = nn.initializers.constant(
                dt_init_std, dtype=self.param_dtype
            )
    ...
    def __call__(self, input_states, cache_params=None):
        ...
        scan_output = scan_output + (
            hidden_states * self.D[jnp.newaxis, :, jnp.newaxis]
        )
...
class FlaxMambaPretrainedModel(BaseNNXModule):
