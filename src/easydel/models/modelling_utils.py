...
def _set_attrs_smartly(self, attr_name: str, default: Any, new_attr: Any):
...
class EDPretrainedConfig(PretrainedConfig):
    ...
    def mesh(self):
        """
        is a helper property that creates a Mesh object from the
    ...
    def get_partition_rules(self, fully_sharded_data_parallel: bool = True):
        """
        The get_partition_rules function is used to specify how the parameters of
        a model are partitioned across devices.
    ...
    def add_basic_configurations(
        ...
        block_b: int = ...,
        block_k_major: int = ...,
        block_q_major_dkv: Optional[int] = ...,
        block_k_major_dkv: Optional[int] = ...,
        block_k_dkv: Optional[int] = ...,
        block_q_dkv: Optional[int] = ...,
        block_k_major_dq: Optional[int] = ...,
        block_k_dq: Optional[int] = ...,
        block_q_dq: Optional[int] = ...,
        ...
        in generation process
        """
        _set_attrs_smartly(self, "axis_dims", (1, -1, 1, 1), axis_dims)
        _set_attrs_smartly(self, "axis_names", ("dp", "fsdp", "tp", "sp"), axis_names)

        _set_attrs_smartly(self, "block_q", 1024, block_q)
        _set_attrs_smartly(self, "block_k", 1024, block_k)
        _set_attrs_smartly(self, "block_b", 1024, block_b)

        _set_attrs_smartly(self, "partition_axis", PartitionAxis(), partition_axis)

        _set_attrs_smartly(
            self, "use_sharding_constraint", False, use_sharding_constraint
        )
        _set_attrs_smartly(self, "backend", jax.default_backend(), backend)
        _set_attrs_smartly(
            self, "shard_attention_computation", True, shard_attention_computation
        )
        _set_attrs_smartly(self, "use_sharded_kv_caching", True, use_sharded_kv_caching)
        _set_attrs_smartly(self, "attn_mechanism", "sharded_vanilla", attn_mechanism)

        _set_attrs_smartly(
            self, "block_k_dkv", block_k_dkv or self.block_k, block_k_dkv
        )
        _set_attrs_smartly(
            self, "block_q_dkv", block_q_dkv or self.block_q, block_q_dkv
        )
        ...
        _set_attrs_smartly(self, "block_k_dq", block_k_dq or self.block_k, block_k_dq)
        _set_attrs_smartly(self, "block_q_dq", block_q_dq or self.block_q, block_q_dq)

        _set_attrs_smartly(self, "easy_method", EasyMethod.TRAIN, easy_method)
        _set_attrs_smartly(self, "bits", None, bits)
        _set_attrs_smartly(self, "scan_attention_layers", True, scan_attention_layers)
        _set_attrs_smartly(self, "scan_ring_attention", True, scan_ring_attention)
        _set_attrs_smartly(self, "use_scan_mlp", True, use_scan_mlp)
        _set_attrs_smartly(self, "scan_mlp_chunk_size", 1024, scan_mlp_chunk_size)
        _set_attrs_smartly(self, "attention_axis_name", "sp", attention_axis_name)
        _set_attrs_smartly(self, "quantize_kv_cache", False, quantize_kv_cache)
        _set_attrs_smartly(
            self,
