...
class LTBlock(nn.Module):
	...
	def setup(self) -> None:
		...
		self.mlp = LTMlp(config=self.config, param_dtype=self.param_dtype, dtype=self.dtype)
...
class FlaxLTForCausalLMModule(nn.Module):
	...
	def __call__(
		...
		hidden_states = (
			base_model_prediction.hidden_states if return_dict else base_model_prediction[1]
		)
