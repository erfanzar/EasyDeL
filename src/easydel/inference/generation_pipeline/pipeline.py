...
class GenerationPipeline:
		...
		**kwargs,  # Ignored arguments.
		...
		with mesh:
			input_ids = fjformer.with_sharding_constraint(
				input_ids,
				PartitionSpec(partition_axes.batch_axis, partition_axes.key_sequence_axis),
			)
		...
		def should_continue_sampling(state):
			...
			return ~jnp.logical_or(all_sequence_finished, state.current_length >= max_length)
	...
	def save_aot_functions(self, path: str):
		...
		if self.compiled_sample_fn_over is not None:
			serialized_function, in_tree, out_tree = serialize(self.compiled_sample_fn_over)
...
class ChatPipeline:
	...
	def stream_generate(self, conversation):
		...
		for sequence in self.pipeline.generate(echo=True, **inputs):
			decoded_sequence = self.pipeline.tokenizer.decode(sequence[0])[captured_length:]
