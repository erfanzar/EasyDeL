...
class TrainArguments(OrderedDict):
    ...
    def __init__(
        ...
        max_length_decp = kwargs.pop("max_length", None)
        if max_length_decp is not None:
            warnings.warn(
                "`max_length` is removed, use `max_sequence_length` instead of that"
            )
            self.max_sequence_length = max_length_decp
