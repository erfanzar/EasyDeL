...
def _rms_norm_forward_kernel(w_ref, x_ref, o_ref, r_ref, *, eps, dim, exp_tun):
...
def _rms_norm_backward_kernel(
	g_ref, x_ref, w_ref, r_ref, dx_ref, dw_ref, *, eps, dim, exp_tun
):
	normed = x_ref[...] * r_ref[...]
	...
	grad_X_R_from_inv_var = (
		-0.5
		* grad_inv_var
		* (
			x_ref[...]
			/ (jnp.sum(x_ref[...] * x_ref[...], -1, keepdims=True) / dim + eps) ** 1.5
		)
	)
...
def _rms_norm_forward_main(
	X: jax.Array, W: jax.Array, eps: float, interpret: bool = False, exp_tun: bool = False
):
	B, S, DIM = X.shape
	W = W.reshape(1, -1)
	X = X.reshape(-1, DIM)
...
def rms_norm(
	...
	return _rms_norm_forward_main(
		W=W, X=X, eps=eps, interpret=interpret, exp_tun=exp_tun
	)[0]
...
if __name__ == "__main__":
	...
	norm = RMSNorm(64, 1e-6, jnp.float16, jnp.float16)
	...
	norm_kernel_out, grad = jax.value_and_grad(
		lambda w: jnp.mean(rms_norm(X=inputs, W=w, eps=1e-6, interpret=True))
	)(
