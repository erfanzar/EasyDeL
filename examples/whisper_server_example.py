...
def main():
	...
	print(
		f"response = requests.post('http://localhost:{args.port}/v1/audio/transcriptions', files=files, data=data)"
	)
	...
	print("data = {'model': 'whisper-large-v3-turbo', 'response_format': 'json'}")
	print(
		f"response = requests.post('http://localhost:{args.port}/v1/audio/translations', files=files, data=data)"
	)
	print("print(json.dumps(response.json(), indent=2))")
	print("------------------------------------------")
