...
def main():
    ...
    def make_fake_input_data(
        ...
        q = random.normal(
            next(rng_gen),
            (batch_size, context_length, num_attention_head, head_dim),
            dtype=jnp.float32,
        )
        k = random.normal(
            next(rng_gen),
        ...
        at_ = jnp.broadcast_to(
            jnp.expand_dims(attention_mask, axis=(-3, -2)), cm_.shape
        )
        ...
        return (q, k, v, attention_mask, causal_mask, attention_bias)

    q, k, v, attention_mask, causal_mask, attention_bias = make_fake_input_data(
        BATCH_SIZE, NUM_ATTN_HEADS, CONTEXT_LENGTH, HEAD_DIM
    )
    ...
    print(
        flash_attn_out.attention_outputs[
            0, CONTEXT_LENGTH - 5, NUM_ATTN_HEADS - 1, HEAD_DIM - 10 :
        ]
    )
    ...
    print(
        normal_attn_out.attention_outputs[
            0, CONTEXT_LENGTH - 5, NUM_ATTN_HEADS - 1, HEAD_DIM - 10 :
        ]
    )
