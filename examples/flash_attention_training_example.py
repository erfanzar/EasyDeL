...
def launch():
    pretrained_model_name_or_path = (
        "openlm-research/open_llama_3b_v2"  # You can change this to any other models
    )
    ...
    config.add_basic_configurations(
        attn_mechanism="flash", block_b=1, block_q=512, block_k=512, block_k_major=512
    )

    original_max_position_embeddings = config.max_position_embeddings
    config.freq_max_position_embeddings = config.max_position_embeddings
    config.max_position_embeddings = 2048
    config.causal_mask_max_position_embeddings = config.max_position_embeddings
