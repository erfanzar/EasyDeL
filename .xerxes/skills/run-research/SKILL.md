...
- Add `allowed-tools` only when the task needs a constrained tool surface, as in `.xerxes/skills/review-pr/SKILL.md`.
...
- Point symptoms to `.xerxes/ops/OPS.md` when they are operational runbooks.
...
2. Read the touched package's `pyproject.toml` and relevant docs under
   `libs/<package>/docs/`.
3. If the task is operational or failure-prone, read `.xerxes/ops/OPS.md`.
4. If the task needs test selection, load
   `.xerxes/skills/test-workspace/SKILL.md`.
...
4. Run focused verification first. Use
   `.xerxes/skills/test-workspace/SKILL.md` for test selection.
5. Keep only changes supported by the verification result.
6. Record durable design or research notes in `.xerxes/projects/` only when the task is too large to finish in the
   current pass. Operational recovery steps belong in `.xerxes/ops/OPS.md`.

## Project Notes

For long-running work, create a note under `.xerxes/projects/<topic>.md` with:
