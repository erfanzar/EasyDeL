...
Load and follow `run-research` first. For TPU availability or bad-node symptoms, read `.xerxes/ops/OPS.md`.
...
If the run is driven by eLarge, also load `.xerxes/skills/train-elarge/SKILL.md`. If the issue involves fused or
...
- Optimizer temps: inspect optimizer state shape and sharding, then load
  `.xerxes/skills/eformer-checkpoint-sharding/SKILL.md` if serialization or restore layout is involved.
