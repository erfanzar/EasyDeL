...
- **Workspace-level**: `WORKSPACE.md` (architecture, authoritative),
  `XERXES.md`, `XERXES.md`, `.xerxes/ops/OPS.md`,
  `.xerxes/skills/*/SKILL.md`, `.xerxes/repo-map.yaml`.

## Workflow

1. Identify the renamed/moved/added symbols from the diff.
2. Grep every doc surface for the old names — including `.xerxes/skills`
   and `.xerxes/agents`, which cite exact paths and symbols and go stale silently:

   ```bash
   rg "<old_symbol>|<old_path>" libs/*/docs WORKSPACE.md XERXES.md XERXES.md .agents
   ```
