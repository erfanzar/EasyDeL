...
def _generate_qkv(
	...
	for _ in range(batch_size):
		seq_global_indices = jnp.arange(
			global_page_offset, global_page_offset + pages_per_sequence, dtype=jnp.int32
		)
...
def _grouped_query_attention_reference(
	...
	logits = jnp.einsum(
		"bhgd,bhtd->bhgt", q_grouped.astype(jnp.float32), k.astype(jnp.float32)
	)
...
def test_paged_attention(
	...
	seq_lens = TEST_SEQ_LENS[TEST_SEQ_LENS <= MAX_SEQ_LEN]  # Filter lengths based on max
	batch_size = len(seq_lens)
	...
	o_ref = _grouped_query_attention_reference(
		q, k_ref, v_ref, seq_lens, attn_logits_soft_cap
	)
