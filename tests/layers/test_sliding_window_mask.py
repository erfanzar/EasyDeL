...
def _visible_positions(mode, query_length, key_length, index, sliding_window, cache_metadata=None):
    ...
    cache_view = types.SimpleNamespace(indexs=jnp.asarray([index], dtype=jnp.int32))
...
def test_decode_metadata_tracks_the_sliced_window():
    """`starts`/`indexs` must point at the valid tokens inside the sliced window."""
    key_length, window, index = 16, 3, 2
    metadata = TransformerMetadata(
        starts=jnp.asarray([0], dtype=jnp.int32),
        indexs=jnp.asarray([index], dtype=jnp.int32),
    )
    ...
    stop = int(np.asarray(out_metadata.indexs)[0])
