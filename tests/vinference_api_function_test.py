...
def test_streaming_completion():
	...
	with requests.post(
		...
		if response.status_code == 200:
			...
			print(
				f"✗ Streaming completion request failed with status code {response.status_code}"
			)
			print(response.text)
...
def test_client_usage():
	...
	try:
		from easydel.inference.vinference.api_server.api_client import vInferenceClient
		from easydel.inference.vinference.api_server.api_models import (
		...
		completion_request = CompletionRequest(
			model="LLaMA", prompt="Complete this: The sky is", max_tokens=16
		)
