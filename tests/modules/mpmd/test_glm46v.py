...
class TestGlm46V:
    """Test suite for GLM46V vision-language model."""

    @pytest.fixture
    def hf_glm46v_class(self):
        return getattr(transformers, "Glm46VForConditionalGeneration", None)
    ...
    @pytest.fixture
    def vlm_config(self, glm46v_config, small_model_config):
        """Create VLM-specific config for GLM46V."""
        num_images_per_batch = 1
        batch_size = small_model_config["batch_size"]
        grid_h, grid_w = 14, 14

        spatial_merge_size = glm46v_config.vision_config.spatial_merge_size
        ...
        in_channels = glm46v_config.vision_config.in_channels
        patch_size = glm46v_config.vision_config.patch_size
        temporal_patch_size = glm46v_config.vision_config.temporal_patch_size or 2
    ...
    def test_vision_language(self, glm46v_config, small_model_config, vlm_config, hf_glm46v_class, mpmd_schedule_kind):
        """Test Glm46VForConditionalGeneration with vision inputs."""
        ...
        result = tester.run(
            module_name="glm46v",
            hf_class=hf_glm46v_class,
            task=ed.TaskType.IMAGE_TEXT_TO_TEXT,
            config=glm46v_config,
            small_model_config=local_cfg,
            vlm_config=vlm_config,
        )
        assert result.success, f"GLM46V VLM failed: {result.error_message or result.comparison.details}"

    @pytest.mark.parametrize("mpmd_schedule_kind", LOSS_SCHEDULE_KINDS, indirect=True)
    def test_text_only(self, glm46v_config, small_model_config, hf_glm46v_class, mpmd_schedule_kind):
        """Test GLM46V text-only forward pass."""
        local_cfg = small_model_config.copy()
        local_cfg["max_position_embeddings"] = 2048
        ...
        assert result.success, f"GLM46V text-only failed: {result.error_message or result.comparison.details}"

    @pytest.mark.parametrize("mpmd_schedule_kind", [GENERATION_SCHEDULE_KIND], indirect=True)
    def test_generation(self, glm46v_config, small_model_config, hf_glm46v_class, mpmd_schedule_kind):
        """Test GLM46V text-only generation."""
        local_cfg = small_model_config.copy()
        local_cfg["max_position_embeddings"] = 2048
        ...
        assert result.success, f"GLM46V generation failed: {result.error_message}"
