...
class TestGlm4v:
    """Test suite for GLM4V vision-language model."""

    @pytest.fixture
    def hf_glm4v_class(self):
        return getattr(transformers, "Glm4vForConditionalGeneration", None)
    ...
    @pytest.fixture
    def vlm_config(self, glm4v_config, small_model_config):
        """Create VLM-specific config for GLM4V."""
        num_images_per_batch = 1
        batch_size = small_model_config["batch_size"]
        grid_h, grid_w = 14, 14

        spatial_merge_size = glm4v_config.vision_config.spatial_merge_size
        ...
        in_channels = glm4v_config.vision_config.in_channels
        patch_size = glm4v_config.vision_config.patch_size
        temporal_patch_size = glm4v_config.vision_config.temporal_patch_size or 2
    ...
    def test_vision_language(self, glm4v_config, small_model_config, vlm_config, hf_glm4v_class, mpmd_schedule_kind):
        """Test Glm4vForConditionalGeneration with vision inputs."""
        ...
        result = tester.run(
            module_name="glm4v",
            hf_class=hf_glm4v_class,
            task=ed.TaskType.IMAGE_TEXT_TO_TEXT,
            config=glm4v_config,
            small_model_config=local_cfg,
            vlm_config=vlm_config,
        )
        assert result.success, f"GLM4V VLM failed: {result.error_message or result.comparison.details}"

    @pytest.mark.parametrize("mpmd_schedule_kind", LOSS_SCHEDULE_KINDS, indirect=True)
    def test_text_only(self, glm4v_config, small_model_config, hf_glm4v_class, mpmd_schedule_kind):
        """Test GLM4V text-only forward pass."""
        local_cfg = small_model_config.copy()
        local_cfg["max_position_embeddings"] = 2048
        ...
        assert result.success, f"GLM4V text-only failed: {result.error_message or result.comparison.details}"

    @pytest.mark.parametrize("mpmd_schedule_kind", [GENERATION_SCHEDULE_KIND], indirect=True)
    def test_generation(self, glm4v_config, small_model_config, hf_glm4v_class, mpmd_schedule_kind):
        """Test GLM4V text-only generation."""
        local_cfg = small_model_config.copy()
        local_cfg["max_position_embeddings"] = 2048
        ...
        assert result.success, f"GLM4V generation failed: {result.error_message}"
