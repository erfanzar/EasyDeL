...
Act as the `architect` agent (`.claude/agents/architect.md`). Read WORKSPACE.md, CLAUDE.md, and `.claude/repo-map.yaml`,
then inspect the real code surfaces the feature touches (ground every path with rg or by opening it — no guessed file
names).

Deliver:

1. Goal in one sentence and the chosen extension point (s) — registry entry, mixin hook, config surface, or new module
   following an existing pattern.
2. Package ownership decision (foundation lib vs easydel) with the layering contract respected.
