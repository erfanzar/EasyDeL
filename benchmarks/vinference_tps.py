...
def plot_results(results_df, output_dir="plots"):
	...
	pivot_table = avg_results.pivot(
		index="prefill_length", columns="max_new_tokens", values="tokens_per_second_mean"
	)
	...
	plt.figure(figsize=(10, 8))
	pivot_table_time = avg_results.pivot(
		index="prefill_length", columns="max_new_tokens", values="total_time_mean"
	)
	...
	plt.figure(figsize=(10, 8))
	avg_results["generation_time"] = (
		avg_results["tokens_generated_mean"] / avg_results["tokens_per_second_mean"]
	)
	avg_results["prefill_time"] = (
		avg_results["total_time_mean"] - avg_results["generation_time"]
	)
	avg_results["prefill_ratio"] = (
		avg_results["prefill_time"] / avg_results["total_time_mean"] * 100
	)

	pivot_prefill_ratio = avg_results.pivot(
		index="prefill_length", columns="max_new_tokens", values="prefill_ratio"
	)
	...
	results_df["config"] = (
		results_df["prefill_length"].astype(str)
		+ "_"
		+ results_df["max_new_tokens"].astype(str)
	)

	config_avg_tps = (
		results_df.groupby("config")["tokens_per_second"]
		.mean()
...
def main():
	...
	parser.add_argument(
		"--model", default="Qwen/Qwen2.5-7B-Instruct-1M", help="Model name or path"
	)
	parser.add_argument(
		"--prefill_lengths",
		nargs="+",
