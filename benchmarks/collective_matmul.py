...
class MatrixMultiplyBenchmark:
    ...
    def _generate_test_matrices(self, m: int, k: int, n: int) -> tuple[jax.Array, jax.Array]:
        """
    ...
    def _benchmark_distributed_matmul(
        ...
        if method == MatrixMultiplyMethod.ALL_GATHER:
            ...
            prepared_right = prepare_matrix_for_all_gather(sharded_right, self.device_mesh, self.mesh_axis)
            in_specs = (
            ...
            sharded_right = jax.device_put(right_matrix, NamedSharding(self.device_mesh, Ps(self.mesh_axis, None)))
            prepared_right = prepare_matrix_for_reduce_scatter(sharded_right, self.device_mesh, self.mesh_axis)
            in_specs = (
                Ps(("sp", "fsdp"), self.mesh_axis),
    ...
    def plot_results(self, save_path: str | None = None, figsize: tuple[int, int] = (12, 8)) -> None:
        ...
        # Plot speedups
        ag_speedups = [s / a for s, a in zip(self.results["standard"], self.results["all_gather"], strict=False)]
        rs_speedups = [s / r for s, r in zip(self.results["standard"], self.results["reduce_scatter"], strict=False)]
    ...
    def print_summary(self) -> None:
        ...
        avg_reduce_scatter = sum(self.results["reduce_scatter"]) / len(self.results["reduce_scatter"])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark distributed matrix multiplication methods")
    parser.add_argument(
        "--sizes",
        type=str,
    ...
    parser.add_argument("--plot", type=str, default=None, help="Path to save benchmark plot")
    parser.add_argument(
        "--dtype",
        type=str,
