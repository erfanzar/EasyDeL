...
def main():
    ...
    rab = XRapTure(config=rab_config)

    rapture_modules = rab.apply_lora(module=model, parameters={"params": params}, tx=tx)
    ...
    for _ in pbar:
        lora_parameters, lora_opt_state, loss = update_fn(
            lora_parameters, lora_opt_state, example_data
        )
        pbar.set_postfix(loss=loss, total_iterations=iterations)

    final_predictions = lora_module(example_data, params=lora_parameters).logits

    merged_params = rab.merge_parameters(lora_parameters)
