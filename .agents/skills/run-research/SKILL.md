...
3. Keep lifecycle rules here. Keep package-specific commands, symptoms, and
   examples in the specialization or in `.agents/ops/OPS.md`.
...
- Point symptoms to `.agents/ops/OPS.md` when they are operational runbooks.
...
2. Read the touched package's `pyproject.toml` and relevant docs under
   `libs/<package>/docs/`.
3. If the task is operational or failure-prone, read `.agents/ops/OPS.md`.
4. If the task needs test selection, load
   `.agents/skills/test-workspace/SKILL.md`.
...
4. Run focused verification first. Use
   `.agents/skills/test-workspace/SKILL.md` for test selection.
5. Keep only changes supported by the verification result.
6. Record durable design or research notes in `.agents/projects/` only when the
   task is too large to finish in the current pass. Operational recovery steps
   belong in `.agents/ops/OPS.md`.
