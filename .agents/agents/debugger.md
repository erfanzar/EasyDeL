...
cadence: `.agents/skills/run-research/SKILL.md`. Infrastructure symptoms
(bad TPU node, libtpu lock, disk pressure) route to `.agents/ops/OPS.md`
...
- **eSurge wrong tokens / cache corruption**: `PageTable.commit()` missing
  ...
  `.agents/ops/OPS.md` and `.agents/skills/debug-esurge/SKILL.md`.
- **OOM / compile blowups**: `.agents/skills/debug-training-oom/SKILL.md` —
...
- For long hunts, record baseline command, hypotheses tested, and negative
  results in `.agents/projects/<topic>.md`.
