...
class Qwen3VLMoeTextMLP(spx.Module):
    ...
    def reform_param(self):
        ...
        Returns:
            ...
            :attr:`gate_up_proj` ColumnParallelLinear, allowing
            HuggingFace-style separate gate / up weights to be merged
            into the fused layout used here (and back again).
        """
...
class Qwen3VLMoeTextModel(EasyDeLBaseModule):
    ...
    def forward(
        ...
        # Deepstack injection indexes a per-layer Python list, so it also
        # requires the trace path.
        trace_layers = self._layer_scan_trace(
            False,
            output_hidden_states=output_hidden_states,
            output_attentions=output_attentions,
            cache_views=views,
            extra=needs_trace_cache or bool(output_router_logits) or deepstack_visual_embeds is not None,
        )
        ...
        def _layer_loop(block, carry):
            ...
            the updated carry tuple ``(hidden_states, cv,
            all_hidden_states, all_attentions, all_router_logits, idx)``.
            """
