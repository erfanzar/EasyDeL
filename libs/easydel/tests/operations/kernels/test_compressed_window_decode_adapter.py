...
"""Tests for the CompressedWindowDecodeAttn EasyDeL operation adapter.

Validates registry wiring, the public-ejkernel-only import boundary, and that
the adapter reproduces an independent dense reference for the DeepSeek-V4
sink-augmented shared-KV decode attention.
"""
...
def test_registered_in_operation_registry():
    ...
    from easydel.operations.kernels import CompressedWindowDecodeAttn

    assert CompressedWindowDecodeAttn.get_impl_name() == "compressed_window_decode"
    assert OperationRegistry.get("compressed_window_decode") is CompressedWindowDecodeAttn


def test_no_private_ejkernel_imports():
    """The adapter imports ejkernel only through public module surfaces."""
    from easydel.operations.kernels import compressed_window_decode as adapter_mod
    ...
    assert "from ejkernel.modules import compressed_window_decode" in src
...
def test_forward_matches_dense_reference(use_sinks):
    """forward_native reproduces the dense reference in fp32."""
    from easydel.operations.kernels import CompressedWindowDecodeAttn

    op = CompressedWindowDecodeAttn(_metadata())
    batch, heads, q_len, head_dim, kv_len = 2, 4, 1, 32, 20
    ...
    out = op(q, kv, bias, sinks, softmax_scale=scale).attention_outputs
    out = np.asarray(out)
