...
class TestQwen3Moe:
    ...
    def test_causal_lm(self, qwen3_moe_config, small_model_config, mpmd_schedule_kind):
        """Test Qwen3MoeForCausalLM."""
        # Use XLA GMM to avoid Pallas TPU grouped_matmul kernel constraints
        qwen3_moe_config.moe_force_xla_gmm = True
        tester = CausalLMTester()
        result = tester.run(
            module_name="qwen3_moe",
            hf_class=transformers.Qwen3MoeForCausalLM,
            task=ed.TaskType.CAUSAL_LM,
            config=qwen3_moe_config,
            small_model_config=small_model_config,
        )
        assert result.success, f"Qwen3Moe CAUSAL_LM failed: {result.error_message or result.comparison.details}"

    @pytest.mark.parametrize("mpmd_schedule_kind", [GENERATION_SCHEDULE_KIND], indirect=True)
    def test_generation(self, qwen3_moe_config, small_model_config, mpmd_schedule_kind):
        ...
        assert result.success, f"Qwen3Moe generation failed: {result.error_message}"
