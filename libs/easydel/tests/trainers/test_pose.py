...
def test_example_B_packed_seqlens_padded_tail():
    ...
    apply_pose_to_positions(
        pos, config=cfg, step=0, rng=rng, tokens=tokens, labels=labels, pad_id=0,
        seq_lens=seq_lens, seq_lens_num=seq_lens_num,
    )
...
def test_core_image_token_skip():
    ...
    apply_pose_to_positions(
        pos, config=cfg, step=0, rng=rng, tokens=input_ids,
        seq_lens=seq_lens, seq_lens_num=seq_lens_num, image_token_id=999,
    )
