...
"""Registry interface for the XLA GDN speculative-window state-scan kernel.

Registers the pure-JAX reference implementation of ``gdn_spec_window_states``
under ``Platform.XLA`` / ``Backend.ANY`` so it serves as the universal
fallback (and the correctness reference for fused backends).
"""
...
from ._xla_impl_fwd import gdn_spec_window_states as _gdn_spec_window_states_xla


@kernel_registry.register("gdn_spec_window_states", Platform.XLA, Backend.ANY)
...
def gdn_spec_window_states(
    ...
    *,
    ...
    """Run the GDN speculative-window state scan via the XLA/JAX reference.

    Registry-facing, runtime-typechecked wrapper; all numerics live in
    :mod:`._xla_impl_fwd`. See that module for the recurrence definition.
    ...
    """
    return _gdn_spec_window_states_xla(
        mixed_qkv,
        b,
        a,
        A_log,
