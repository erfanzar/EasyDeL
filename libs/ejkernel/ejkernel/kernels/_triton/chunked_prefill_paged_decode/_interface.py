...
"""Chunked prefill + paged decode attention implemented in Triton (GPU).

This op updates a block-tabled KV cache with the provided `keys` / `values`,
then computes unified paged attention outputs for the packed `queries`.
"""
...
from ._triton_impl_fwd import chunked_prefill_paged_decode_triton


@kernel_registry.register("chunked_prefill_paged_decode", Platform.TRITON, Backend.GPU)
...
def chunked_prefill_paged_decode(
    ...
    *,
    ...
    """Triton GPU implementation of chunked prefill + paged decode attention.

    Updates a block-tabled KV cache with new key/value pairs, then computes
    causal attention outputs for packed queries against the updated cache.
    ...
    Args:
        ...
        alibi_slopes: Optional ALiBi positional bias slopes [num_q_heads]
        ...
        logits_soft_cap: Optional logit capping for stability
        seq_threshold_3d: Sequence threshold for 3D kernel variant
    ...
    Raises:
        NotImplementedError: If causal=False (only causal attention is supported)
        ValueError: If input shapes are inconsistent
    """
    if softmax_scale is None:
        softmax_scale = 1.0 / math.sqrt(queries.shape[-1])
    return chunked_prefill_paged_decode_triton(
        queries=queries,
        keys=keys,
        values=values,
        key_cache=key_cache,
