...
def _reorder_concatenated_tensor_for_sharding(
    ...
    """Arrange fused ``[A|B|C]`` features into per-shard interleaved order.

    Given a tensor whose ``dim`` axis is the concatenation of several feature
    blocks ``[A | B | C | ...]`` (sizes given by ``split_sizes``), this splits
    each block into ``n_shards`` equal sub-blocks and reinterleaves them so the
    result is laid out as ``[A0 | B0 | C0 | A1 | B1 | C1 | ...]`` along ``dim``.
    This matches the layout a tensor-parallel mesh expects when the fused
    projection is sharded across ``n_shards`` devices. The output keeps the
    original shape; only the ordering along ``dim`` changes.

    Args:
        concatenated_tensor: Source tensor containing the concatenated feature
            blocks along ``dim``.
        split_sizes: Per-block sizes along ``dim`` whose sum equals
            ``concatenated_tensor.shape[dim]``. Each block must be divisible by
            ``n_shards``.
        n_shards: Number of tensor-parallel shards to interleave the blocks
            into.
        dim: Axis along which the blocks are concatenated. Negative values are
            normalized against ``concatenated_tensor.ndim``.

    Returns:
        jax.Array: Tensor with the same shape and dtype as
        ``concatenated_tensor`` whose ``dim`` axis is reordered into
    ...
    *,
    ...
    Args:
        ...
        distribution: ``int32[3]`` ``(decode_end, prefill_end, mixed_end)``
            ...
            ``decode_only_branch`` / ``mixed_prefill_branch`` selection. The
            branch is decode-only when ``distribution[0] == distribution[2]``.
        has_initial_state: Optional boolean array ``(num_requests,)`` marking
            which requests already carry recurrent state. Defaults to all-True
            (every slot has initial state) when ``None``. Only consulted by the
            recurrent-scan prefill path.
        n_kq: Number of key/query heads (before head expansion). Static.
        n_v: Number of value heads (after the GQA-style expansion). Static.
        d_k: Per-head key/query dimension. Static.
        d_v: Per-head value dimension. Static.
        chunk_size: Padding granularity used by the mixed-prefill branch.
            Static; defaults to 64.
        use_qk_norm_in_gdn: Whether to L2-normalize queries and keys. Static;
            defaults to True.
        apply_silu_in_gdr: When True, apply SiLU to ``mixed_qkv`` before
            splitting it into Q/K/V. Static; defaults to False.
        use_recurrent_scan_prefill: When True (and running on TPU), the
    ...
    def mixed_prefill_branch(_):
        ...
        if use_recurrent_scan_prefill and jax.default_backend() == "tpu":
            ...
            def recurrent_prefill_only(_):
                """Run the recurrent-scan kernel for the pure-prefill case.

                Selected under ``lax.cond`` when ``distribution[0] == 0`` (no
                decode tokens), delegating to :func:`recurrent_scan` with
                ``prefill_only=True``.
            ...
            def jax_mixed_prefill(_):
                """Run the chunked GDR prefill for the mixed decode+prefill case.

                Selected under ``lax.cond`` when ``distribution[0] != 0`` (the
                step contains both decode and prefill tokens), delegating to
                :func:`ragged_gated_delta_rule_mixed_prefill`.
...
def ragged_gated_delta_rule_v2(
    ...
    *,
    ...
    Thin wrapper over :func:`ragged_gated_delta_rule` that first casts all
    floating-point inputs to a common ``runtime_dtype`` (so callers may pass
    mixed-precision tensors), defaults ``has_initial_state`` to all-True, and
    then dispatches to the JIT-compiled entry point.
    ...
    Args:
        ...
        a: Pre-softplus alpha source ``(num_tokens, n_v)``.
        recurrent_state: Global state pool ``(num_blocks, n_v, d_k, d_v)``;
            block 0 is the null block reserved for padded slots.
        A_log: Per-head log-decay ``(n_v,)``.
        dt_bias: Per-head delta-time bias ``(n_v,)``.
        query_start_loc: Cumulative per-request offsets,
            shape ``(num_requests + 1,)``.
        state_indices: Request-to-slot mapping, shape ``(num_requests,)``.
        distribution: ``int32[3]`` ``(decode_end, prefill_end, mixed_end)``
            classifying scheduled requests.
        has_initial_state: Optional boolean array ``(num_requests,)`` marking
            which requests carry recurrent state. Defaults to all-True when
            ``None``.
    ...
    Returns:
        tuple: ``(updated_recurrent_state, output)`` with state of shape
        ``(num_blocks, n_v, d_k, d_v)`` and output of shape
        ``(num_tokens, n_v * d_v)``, as returned by
