...
def test_gradient_matches_reference(batch, heads, q_len, head_dim, kv_len):
    ...
    def _ref(qq, kk, ss):
        # Match the XLA kernel's explicit full-fp32 dot contract. Float32
        # operands alone still use reduced-precision MXU products by default
        # on TPU, including the transposed dots generated by autodiff.
        logits = jnp.einsum("bhsd,bld->bhsl", qq, kk, precision=jax.lax.Precision.HIGHEST).astype(jnp.float32)
        ...
        return jnp.sum(jnp.einsum("bhsl,bld->bhsd", p, kk, precision=jax.lax.Precision.HIGHEST) * cot)
