...
compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh,
  ...
  const index_t row_offset_lse =
      ...
      (m_block_max - 1) * kBlockM;
  // Regarding 128 * params.b see a comment in mha_varlen_bwd about padding of
  // dq_accum and softmax_d
...
#pragma unroll
  ...
  FLASH_NAMESPACE::Dropout dropout(params.rng_state[0], params.rng_state[1],
                                   params.p_dropout_in_uint8_t, bidb, bidh,
                                   tidx, params.h);
  ...
  FLASH_NAMESPACE::Alibi<Is_causal> alibi(alibi_slope, binfo.actual_seqlen_k,
...
#pragma unroll
    ...
    // Reshape acc_s from (MMA=4, MMA_N, MMA_N) to (row=(2, MMA_N), col=(2,
    // MMA_N))
    Tensor scores =
    ...
    if (!Is_causal && !Is_local) {
      if (!Is_even_MN && (n_block + 1) * kBlockN >= binfo.actual_seqlen_k) {
        FLASH_NAMESPACE::apply_mask(
            scores, binfo.actual_seqlen_k,
            n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16);
      }
    } else if (Is_causal) {
      ...
      if (m_block * kBlockM < (n_block + 1) * kBlockN + binfo.actual_seqlen_q -
          ...
          (!Is_even_MN && (n_block + 1) * kBlockN >= binfo.actual_seqlen_k)) {
        FLASH_NAMESPACE::apply_mask_local(
            scores,
            n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16,
            binfo.actual_seqlen_k, m_block * kBlockM + get<0>(taccScS_row(0)),
    ...
    FLASH_NAMESPACE::scale_apply_exp2</*scale_max=*/false>(
        scores, lse, params.scale_softmax_log2);
    ...
    Tensor tPrP = make_tensor(
        rP.data(),
        FLASH_NAMESPACE::convert_layout_acc_Aregs<Kernel_traits::TiledMmaSdP>(
            rP.layout()));
    Tensor tPaP =
    ...
    // FLASH_NAMESPACE::convert_layout_acc_rowcol(acc_dp.layout())); #pragma
    // unroll for (int mi = 0; mi < size<0>(acc_dp_reshaped); ++mi) {
    //     #pragma unroll
...
#pragma unroll
      for (int ni = 0; ni < size<1>(dS); ++ni) {
        float scaled_ds =
            pointwise_mult(scores(mi, ni), dS(mi, ni), dP_sum(mi));
        if constexpr (Is_softcap) {
    ...
    // get<2>(p_l), get<1>(p_l))); FLASH_NAMESPACE::gemm_rs(acc_dv, tdVrPt,
    // tdVrdO, tdVsdOt, tiled_mma_dkv, smem_thr_copy_QdOt); Tensor tdKrdSt =
    // make_tensor(tdSrdS.data(), tdVrPt.layout());
    // FLASH_NAMESPACE::gemm_rs(acc_dk, tdKrdSt, tdKrQt, tdKsQt, tiled_mma_dkv,
    // smem_thr_copy_QdOt);
    FLASH_NAMESPACE::gemm(acc_dv, tdVrPt, tdVrdO, tdVsPt, tdVsdOt,
                          tiled_mma_dkv, smem_tiled_copy_PdSt,
                          smem_tiled_copy_QdOt, smem_thr_copy_PdSt,
                          smem_thr_copy_QdOt);
...
#pragma unroll
    ...
    FLASH_NAMESPACE::gemm(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt,
                          tiled_mma_dkv, smem_tiled_copy_PdSt,
                          smem_tiled_copy_QdOt, smem_thr_copy_PdSt,
                          smem_thr_copy_QdOt);
