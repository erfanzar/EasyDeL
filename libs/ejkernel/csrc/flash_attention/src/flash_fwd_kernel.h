...
get_lse_tile(const Params &params, const int bidb, const int bidh,
             const int m_block,
    ...
    for (int n_block = n_block_max - 1; n_block >= n_block_copy_min;
      ...
      if (block_table == nullptr) {
        ...
        if (n_block > n_block_copy_min) {
          ...
          const int table_diff = block_table[block_table_idx_next] -
                                 block_table[block_table_idx_cur];
          const int offset_diff =
              block_table_offset_next - block_table_offset_cur;
          tVgV.data() = tVgV.data() + table_diff * params.v_batch_stride +
                        offset_diff * params.v_row_stride;
          tKgK.data() = tKgK.data() + table_diff * params.k_batch_stride +
                        offset_diff * params.k_row_stride;
        }
      }
    }
