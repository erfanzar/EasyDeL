...
template <typename Tensor0, typename Tensor1, typename Tensor2,
          ...
          typename ThrCopy>
__forceinline__ __device__ void
gemm_rs(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsB,
...
#pragma unroll
  for (int i = 0; i < size(tensor); ++i) {
    tensor(i) = cutlass::fast_tanh(tensor(i) / softcap) * softcap;
  }
}
...
template <typename Engine0, typename Layout0, typename Engine1,
          typename Layout1>
__forceinline__ __device__ void
calculate_dtanh(Tensor<Engine0, Layout0> &src_tensor,
                Tensor<Engine1, Layout1> &dst_tensor, const float softcap) {
#pragma unroll
  for (int i = 0; i < size(src_tensor); ++i) {
    const float scaled = src_tensor(i) / softcap;
    dst_tensor(i) = 1.f - (scaled * scaled);
  }
}
