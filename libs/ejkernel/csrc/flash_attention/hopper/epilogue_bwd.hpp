...
struct CollectiveEpilogueBwd {
  ...
  static constexpr bool Use_TMA =
      !Varlen && ArchTag::kMinComputeCapability >= 90;
  ...
  using SmemCopyAtomdKV =
      Copy_Atom<std::conditional_t<
                    ArchTag::kMinComputeCapability >= 90,
                    std::conditional_t<!dKV_swapAB, cute::SM90_U32x4_STSM_N,
  ...
  struct TensorStorage : cute::aligned_struct<SmemAlignmentdKV> {
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutdKV>,
                        SmemAlignmentdKV>
        smem_dk;
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutdKV>,
                        SmemAlignmentdKV>
  ...
  using TMA_dKV = std::conditional_t<
      ...
      decltype(make_tma_copy(
          ...
          make_tensor(make_gmem_ptr(static_cast<Element *>(nullptr)),
                      ShapedKV{}, StridedKV{}),
          SmemLayoutdKVTMA{}, select<1, 2>(TileShape_MNK{}), _1{})), // no mcast
                                                                     // for dKV
      std::nullptr_t>;
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_dKV tma_store_dK = [&] {
      if constexpr (Use_TMA) {
        return make_tma_copy(GmemTiledCopydKVTMA{}, mdK, SmemLayoutdKVTMA{},
                             select<1, 2>(TileShape_MNK{}),
                             _1{}); // no mcast for dKV
      } else {
        return nullptr;
    ...
    TMA_dKV tma_store_dV = [&] {
      if constexpr (Use_TMA) {
        return make_tma_copy(GmemTiledCopydKVTMA{}, mdV, SmemLayoutdKVTMA{},
                             select<1, 2>(TileShape_MNK{}),
                             _1{}); // no mcast for dKV
      } else {
        return nullptr;
    ...
    return {args.ptr_dK,     args.shape_dK,  args.stride_dK, args.ptr_dV,
            args.shape_dV,   args.stride_dV, tma_store_dK,   tma_store_dV,
  ...
  store(Params const &params, FrgTensorO const &tdKrdK,
    ...
    if constexpr (Use_TMA) {
      ...
      flash::SeqlenInfo<Varlen, kBlockN> seqlen_info{
          bidb, size<0>(params.shape_dK), params.cu_seqlens, params.seqused};
      ...
      Tensor mdV =
          make_tensor(make_gmem_ptr(params.ptr_dV), params.shape_dV,
...
#pragma unroll
      for (int k = 0; k < size(tdKVpdV); ++k) {
        tdKVpdV(k) = get<1>(tdKVcdKV(_0{}, _0{}, k)) < get<1>(params.shape_dV);
      }
#pragma unroll
      for (int k = 0; k < size(tdKVpdK); ++k) {
        tdKVpdK(k) = get<1>(tdKVcdKV(_0{}, _0{}, k)) < get<1>(params.shape_dK);
      }
      ...
      flash::copy</*Is_even_MN=*/EvenN, /*Is_even_K=*/true,
                  /*Clear_OOB_MN=*/false>(gmem_tiled_copy_dKV, tdKVsdV, tdKVrdV,
                                          tdKVcdKV, tdKVpdV, kBlockN);
      flash::copy</*Is_even_MN=*/EvenN, /*Is_even_K=*/true,
                  /*Clear_OOB_MN=*/false>(gmem_tiled_copy_dKV, tdKVsdK, tdKVrdK,
                                          tdKVcdKV, tdKVpdK, kBlockN);
      // // Tell warp 0 that smem_k and smem_v are ready
      ...
      flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
          ...
          gmem_tiled_copy_dKV, tdKVrdV, tdKVgdV, tdKVcdKV, tdKVpdV,
          std::min(seqlen_info.seqlen - n_block * kBlockN, kBlockN));
      flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                  /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
          gmem_tiled_copy_dKV, tdKVrdK, tdKVgdK, tdKVcdKV, tdKVpdK,
          std::min(seqlen_info.seqlen - n_block * kBlockN, kBlockN));
    }
  }
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
        gmem_tiled_copy_dKV, tdKVrdKV, tdKVgdK, tdKVcdKV, tdKVpdK,
        seqlen_info.seqlen - n_block * kBlockN);
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
        gmem_tiled_copy_dKV, tdKVrdKV, tdKVgdV, tdKVcdKV, tdKVpdV,
        seqlen_info.seqlen - n_block * kBlockN);
  }
};
...
struct CollectiveEpilogueBwdGQA {
  ...
  static constexpr int kBlockN = get<1>(TileShape_MNK{});
  static constexpr int kHeadDim = get<2>(TileShape_MNK{});
  static_assert(NumEpilogueThreads % cutlass::NumThreadsPerWarp == 0,
                "NumEpilogueThreads must be a multiple of NumThreadsPerWarp");
  static constexpr int NumWarpGroups =
      NumEpilogueThreads / cutlass::NumThreadsPerWarpGroup;
  // Thread layout, 256 or 384 threads per row
  // We split into NumWarpGroups so that we can use the same postprocessing
  // kernel as dQ
  using R2SLayoutAtomdKVaccum =
      Layout<Shape<Int<cutlass::NumThreadsPerWarpGroup>, Int<NumWarpGroups>>>;
  ...
  using R2GTiledCopydKVaccum = decltype(make_tiled_copy(
      Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, ElementAccum>{},
      R2GLayoutAtomdKVaccum{},
      Layout<Shape<_1>>{})); // Val layout, 1 vals per store

  using SmemLayoutdKVaccum = Layout<
      Shape<Int<kBlockN * kHeadDim / NumWarpGroups>, Int<NumWarpGroups>>>;
  using SmemLayoutdKVaccumFlat = Layout<Shape<Int<kBlockN * kHeadDim>>>;

  // Strangely without this SmemAlignment, the total smem for hdim 128 (80 x
  // 128) is 228KB even though we only need 227KB. We use the same alignment as
  // the non-GQA epilogue to avoid this issue.
  static constexpr int SmemAlignment =
      kHeadDim % 64 == 0 ? 1024 : (kHeadDim % 32 == 0 ? 512 : 256);
  struct TensorStorageTMA : cute::aligned_struct<SmemAlignment> {
    cute::array_aligned<ElementAccum, cute::cosize_v<SmemLayoutdKVaccum>,
                        SmemAlignment>
        smem_dkv;
  };
  ...
  using ShapedKV = cute::Shape<int32_t, int32_t,
                               int32_t>; // (seqlen_k_rounded * d, head, batch)
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    return {args.ptr_dKaccum,
            ...
            args.stride_dVaccum,
            cutlass::FastDivmod(
                cute::ceil_div(args.num_heads_q, get<1>(args.shape_dKaccum))),
            args.dk_semaphore,
            args.dv_semaphore,
            args.num_batch,
            args.cu_seqlens,
  ...
  store(Params const &params, FrgTensorO const &tdKrdK,
    ...
    Tensor sdKV = make_tensor(
        make_smem_ptr(shared_storage.tensors.epilogue.smem_dkv.data()),
        SmemLayoutdKVaccum{});
    Tensor sdKV_flat = make_tensor(
        make_smem_ptr(shared_storage.tensors.epilogue.smem_dkv.data()),
        SmemLayoutdKVaccumFlat{});
    static constexpr int dKV_TMA_num_bytes =
        CUTE_STATIC_V(size(sdKV_flat)) * sizeof(ElementAccum);

    flash::SeqlenInfo<Varlen, kBlockN> seqlen_info{
        bidb, size<0>(params.shape_dKaccum), params.cu_seqlens, params.seqused};
    bool const is_varlen = Varlen && params.cu_seqlens;
    Tensor mdKaccum =
        make_tensor(make_gmem_ptr(params.ptr_dKaccum), params.shape_dKaccum,
                    params.stride_dKaccum)(_, bidh_kv, !is_varlen ? bidb : 0);
    Tensor mdVaccum =
        make_tensor(make_gmem_ptr(params.ptr_dVaccum), params.shape_dVaccum,
                    params.stride_dVaccum)(_, bidh_kv, !is_varlen ? bidb : 0);
    Tensor gdKaccum = local_tile(
        domain_offset(make_coord(seqlen_info.offset_padded * kHeadDim),
                      mdKaccum),
        Shape<Int<kBlockN * kHeadDim>>{}, make_coord(n_block)); // (M * K)
    ...
    auto r2s_thr_copy_dKVaccum =
        r2s_tiled_copy_dKVaccum.get_thread_slice(thread_idx);
    ...
    auto r2g_thr_copy_dKVaccum =
        r2g_tiled_copy_dKVaccum.get_thread_slice(thread_idx);
    ...
    flash::named_barrier_sync(
        ...
        cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);
    if constexpr (Use_TMA) {
      Tensor taccdKVrdV = r2s_thr_copy_dKVaccum.retile_S(
          tdVrdV); // ((Atom,AtomNum), MMA_M, MMA_N)
      cute::copy(r2s_tiled_copy_dKVaccum, taccdKVrdV, tdKVsdKVaccum);
    }

    int const num_batch = params.num_batch;
    // int const num_batch = get<2>(params.shape_dKaccum); // erroneously
    // returns 1 for varlen
    ...
    // if (thread_idx == 0) { printf("blockIdx.x = %d, blockIdx.y = %d,
    // blockIdx.z = %d, bidb = %d, bidh_kv = %d, lock_ptr = %p, dv_semaphore =
    // %p, num_batch = %d, num_head_kv = %d, n_block = %d, bihd_idx_in_group =
    // %d\n", blockIdx.x, blockIdx.y, blockIdx.z, bidb, bidh_kv, lock_ptr,
    // params.dv_semaphore, num_batch, num_head_kv, n_block,
    // bidh_idx_in_group);}

    if constexpr (Deterministic) {
      Barrier::wait_eq(lock_ptr, thread_idx, n_block * num_batch * num_head_kv,
                       bidh_idx_in_group);
    }
    // if (thread_idx == 0) { printf("After barrier blockIdx.x = %d, blockIdx.y
    // = %d, blockIdx.z = %d, bidb = %d, bidh_kv = %d, lock_ptr = %p,
    // dv_semaphore = %p\n", blockIdx.x, blockIdx.y, blockIdx.z, bidb, bidh_kv,
    // lock_ptr, params.dv_semaphore);}
    if constexpr (Use_TMA) {
      ...
      if (thread_idx == 0) {
        SM90_BULK_REDUCE_ADD::copy(
            raw_pointer_cast(sdKV_flat.data()),
            raw_pointer_cast(gdVaccum.data()), dKV_TMA_num_bytes,
      ...
      static_assert(CUTE_STATIC_V(size(tdVrdV_atomic)) ==
                    CUTE_STATIC_V(size(tdVgdV_atomic)));
#pragma unroll
    ...
    if constexpr (Deterministic) {
      Barrier::arrive_inc(lock_ptr, thread_idx,
                          n_block * num_batch * num_head_kv);
    }
    ...
    if constexpr (Use_TMA) {
      cutlass::arch::NamedBarrier::sync(
          NumEpilogueThreads,
          cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);
      Tensor taccdKVrdK = r2s_thr_copy_dKVaccum.retile_S(
          tdKrdK); // ((Atom,AtomNum), MMA_M, MMA_N)
      cute::copy(r2s_tiled_copy_dKVaccum, taccdKVrdK, tdKVsdKVaccum);
    }
    lock_ptr = !Deterministic
                   ? nullptr
                   : params.dk_semaphore + bidb * num_head_kv + bidh_kv;
    // if (thread_idx == 0) { printf("blockIdx.x = %d, blockIdx.y = %d,
    // blockIdx.z = %d, bidb = %d, bidh_kv = %d, lock_ptr = %p, dk_semaphore =
    // %p, num_batch = %d, num_head_kv = %d, n_block = %d, bihd_idx_in_group =
    // %d\n", blockIdx.x, blockIdx.y, blockIdx.z, bidb, bidh_kv, lock_ptr,
    // params.dk_semaphore, num_batch, num_head_kv, n_block,
    // bidh_idx_in_group);}
    ...
    if constexpr (Use_TMA) {
      ...
      if (thread_idx == 0) {
        SM90_BULK_REDUCE_ADD::copy(
            raw_pointer_cast(sdKV_flat.data()),
            raw_pointer_cast(gdKaccum.data()), dKV_TMA_num_bytes,
            static_cast<uint64_t>(TMA::CacheHintSm90::EVICT_LAST));
        tma_store_arrive();
        tma_store_wait<0>();
      }
      ...
      static_assert(CUTE_STATIC_V(size(tdKrdK_atomic)) ==
                    CUTE_STATIC_V(size(tdKgdK_atomic)));
#pragma unroll
      for (int i = 0; i < size(tdKrdK_atomic); ++i) {
        atomicAdd(&tdKgdK_atomic(i), tdKrdK_atomic(i));
      }
    }
    if constexpr (Deterministic) {
      Barrier::arrive_inc(lock_ptr, thread_idx,
  ...
  store_zero(Params const &params, int thread_idx,
             cute::tuple<int32_t, int32_t, int32_t> const &block_coord) {
    // Don't need to do anything since dKaccum and dVaccum are already
    // zero-initialized
  }
};
