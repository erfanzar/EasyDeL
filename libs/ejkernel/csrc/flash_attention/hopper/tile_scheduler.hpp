...
struct TileSchedulerArguments {
  ...
  int const seqlen; // Only used if Varlen and cu_seqlens == nullptr and seqused
                    // == nullptr
  int const seqlen_k, headdim, headdim_v,
...
template <bool Varlen = false, bool Split = false, bool PackGQA = false,
          int kBlock = 128>
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    ...
    assert(!Split || !Varlen ||
           args.num_splits <
               (1 << 16)); // We use the top 16 bits to store num_splits
    return {args.num_blocks,
            args.num_head,
            args.num_batch,
            !Split ? 1 : args.num_splits,
            args.qhead_per_khead,
            args.seqlen,
  ...
  static dim3 get_grid_shape(Params const &params, int num_sm) {
    return {uint32_t(params.num_blocks),
  ...
  struct WorkTileInfo {
    ...
    CUTLASS_DEVICE
    bool is_valid(Params const &params) const { return bidb >= 0; }
    ...
    if constexpr (Varlen) {
      int seqlen =
          params.seqused
              ...
              : (params.cu_seqlens ? params.cu_seqlens[work_info.bidb + 1] -
                                         params.cu_seqlens[work_info.bidb]
                                   : params.seqlen);
    ...
    if constexpr (Varlen && Split) {
      int num_splits_dynamic =
          params.num_splits_dynamic_ptr
              ? params.num_splits_dynamic_ptr[work_info.bidb]
              : params.num_splits;
...
template <bool Split = false> class StaticPersistentTileScheduler {
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    return {args.num_blocks * args.num_head * args.num_batch *
            ...
            cutlass::FastDivmod(args.num_head * (!Split ? 1 : args.num_splits)),
            cutlass::FastDivmod(!Split ? 1 : args.num_splits)};
  }
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    long long const size_one_kv_head = long(args.seqlen_k) *
                                       long(args.headdim + args.headdim_v) *
                                       long(args.element_size);
    int const size_l2 = 32 * 1024 * 1024; // 32 MB for K & V
    // Swizzle is the size of each "section". Round swizzle to a power of 2
    // If not PackGQA already, the size of each section can increase by
    // qhead_per_khead Need to be careful about the case where only one head
    // will fit
    auto find_log2_floor = [&](int n) { return 31 - cutlass::clz(n); };
    // Seems faster if swizzle if a power of 2
    int const swizzle =
        (size_l2 < size_one_kv_head
             ? 1
             : (1 << find_log2_floor(size_l2 / size_one_kv_head))) *
    ...
    int const num_split_blocks =
        args.num_blocks * (!Split ? 1 : args.num_splits);
    ...
    return {num_split_blocks * args.num_head * args.num_batch,
            cutlass::FastDivmod(args.num_blocks),
            cutlass::FastDivmod(args.num_head), cutlass::FastDivmod(swizzle),
  ...
  void init_consumer() const {
    if (WarpSpecialized || cutlass::canonical_warp_idx_sync() > 0) {
      flash::named_barrier_arrive(
          NumThreads, cutlass::arch::ReservedNamedBarriers::
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    // Since it's the bwd pass, seqlen_k get passed to args.seqlen and seqlen_q
    // is passed to args.seqlen_k
    long long const size_one_qdo_head = long(args.seqlen_k) *
                                        long(args.headdim + args.headdim_v) *
                                        long(args.element_size);
    ...
    // by swizzle. Instead we want to divide by the remainder.
    int const num_hb_remainder = (args.num_head * args.num_batch) % swizzle;
    // printf("num_blocks = %d, num_head = %d, num_batch = %d, size_one_head =
    // %d, ratio = %d, swizzle = %d, num_hb_remainder = %d\n", args.num_blocks,
    // args.num_head, args.num_batch, size_one_head, size_l2 / size_one_head,
    // swizzle, num_hb_remainder);
    assert(args.tile_count_semaphore != nullptr);
    return {args.num_blocks * args.num_head * args.num_batch,
            cutlass::FastDivmod(args.num_blocks),
            cutlass::FastDivmod(args.num_head), cutlass::FastDivmod(swizzle),
            cutlass::FastDivmod(swizzle * args.num_blocks),
            // don't divide by 0
            cutlass::FastDivmod(num_hb_remainder > 0 ? num_hb_remainder : 1),
            (args.num_head * args.num_batch) / swizzle, args.seqlen,
            !Varlen ? nullptr : args.cu_seqlens,
            !Varlen ? nullptr : args.seqused};
  }
  ...
  CUTLASS_DEVICE WorkTileInfo get_initial_work(Params const &params) const {
    ...
    if constexpr (Varlen) {
      int seqlen = params.seqused
                       ? params.seqused[bidb]
                       : (params.cu_seqlens ? params.cu_seqlens[bidb + 1] -
                                                  params.cu_seqlens[bidb]
                                            : params.seqlen);
...
template <int kBlockM, int kBlockN,
          ...
          int NumProducerThreads = cutlass::NumThreadsPerWarp,
          bool Split = false, bool PackGQA = false, bool WarpSpecialized = true,
          bool LPT = false, bool Sort = false, bool Prepared = true>
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    ...
    assert(args.tile_count_semaphore != nullptr);
    assert(args.num_head <
           (1 << 16)); // We use the top 16 bits to store num_splits & split_idx
    assert(!Split || args.num_splits <
                         (1 << 8)); // We use the top 8 bits to store num_splits
    // int const size_l2 = 50 * 1024 * 1024; // 50 MB
    // int const size_one_kvblock = kBlockN * (args.headdim + args.headdim_v) *
    // args.element_size; int max_kvblocks_in_l2 = size_l2 / size_one_kvblock;
    return {args.num_head, args.num_batch, args.qhead_per_khead, args.seqlen,
            // max_kvblocks_in_l2,
            cutlass::FastDivmod(args.num_head),
            cutlass::FastDivmod(!Split ? 1 : args.num_splits),
            args.tile_count_semaphore, args.cu_seqlens, args.seqused,
            args.num_splits_dynamic_ptr, args.num_m_blocks_ptr,
  ...
  struct WorkTileInfo {
    ...
    get_block_coord(Params const &params) const {
      ...
      if constexpr (!Split) {
        ...
        // the top 8 bits of bidh store num_splits and the next 8 bits store
        // split_idx reinterpret_cast to uint32_t to make sure we're not doing
        // sign extension when we shift
        ...
        // Use the top 16 bits of split_idx to store num_splits and the next 16
        // bits to store split_idx
        uint32_t split_idx_u = ((bidh_packed & 0x00FF0000) >> 16) +
                               ((bidh_packed & 0xFF000000) >> 8);
        int split_idx = reinterpret_cast<int &>(split_idx_u);
        // int bidh_actual = params.nsplits_divmod.divmod(split_idx, bidh);
        // if (threadIdx.x == 128) {
        //     printf("blockIdx.x = %d, bidb = %d, bidh = %d, bidh_actual = %d,
        //     split_idx = %d\n", blockIdx.x, bidb, bidh, bidh_actual,
        //     split_idx);
        // }
  ...
  WorkTileInfo tile_idx_to_work_tile(Params const &params, int next_tile_idx,
    ...
    auto get_num_m_blocks = [&](int bidb_start) {
      ...
      if constexpr (Prepared) {
        ...
        if (seqlen > kBlockM) {
          if (params.seqused) {
            seqlen =
                batch_idx < params.num_batch ? params.seqused[batch_idx] : 0;
          } else if (params.cu_seqlens) {
            int cur_cu_seqlen = batch_idx <= params.num_batch
                                    ? params.cu_seqlens[batch_idx]
                                    : 0;
    ...
    auto get_num_splits = [&](int bidb_start) {
      int batch_idx = lane + bidb_start;
      bool is_valid =
          batch_idx < params.num_batch && lane < cutlass::NumThreadsPerWarp - 1;
      if constexpr (!Split) {
        return is_valid ? 1 : 0;
    ...
    int m_blocks_in_group = __shfl_sync(0xffffffff, num_m_blocks_cumulative,
                                        cutlass::NumThreadsPerWarp - 1);
    // Only the lower 16 bits are the actual bidh
    // int current_bidh = !Split ? current_work.bidh : (current_work.bidh &
    // 0x0000FFFF); int group_end_tile = current_work.tile_idx -
    // current_work.block - current_bidh * __shfl_sync(0xffffffff,
    // num_split_m_blocks, 0 /*lane*/) + m_blocks_in_group * params.num_head; //
    // Same for all lanes if constexpr (Split) {
    //     int current_split_idx = (current_work.bidh & 0x00FF0000) >> 16;
    //     group_end_tile -= current_split_idx * __shfl_sync(0xffffffff,
    //     num_m_blocks, 0 /*lane*/);
    // }
    ...
    //     num_m_blocks = %d, next_tile_idx = %d, cur tile_idx = %d, cur block =
    //     %d, cur bidh = %d, num_split_m_blocks = %d, group_end_tile = %d,
    //     m_blocks_in_group = %d\n", blockIdx.x, threadIdx.x,
    //     current_work.bidb, num_m_blocks, next_tile_idx,
    //     current_work.tile_idx, current_work.block, current_bidh,
    //     num_split_m_blocks, group_end_tile, m_blocks_in_group);
    // }
    // if (threadIdx.x == 0 && blockIdx.x == 0) { printf("tile_idx = %d,
    // group_end_tile = %d, num_m_blocks_cumulative = %d, m_blocks_in_group =
    // %d\n", current_work.tile_idx, group_end_tile, num_m_blocks_cumulative,
    // m_blocks_in_group); }
    while (group_end_tile <= next_tile_idx) {
      ...
      m_blocks_in_group = __shfl_sync(0xffffffff, num_m_blocks_cumulative,
                                      cutlass::NumThreadsPerWarp - 1);
    ...
    int batch_idx_in_group = __popc(
        __ballot_sync(0xffffffff, group_start_tile + num_m_blocks_cumulative *
                                                         params.num_head <=
                                      next_tile_idx));
    // if (threadIdx.x == 31 || threadIdx.x == 0) { printf("blockIdx.x = %d,
    // tidx %d, group_start_tile = %d, num_m_blocks_cumulative = %d, num_head =
    // %d, next_tile_idx = %d, ballot = %x, batch_idx_in_group = %d\n",
    // blockIdx.x, threadIdx.x, group_start_tile, num_m_blocks_cumulative,
    // params.num_head, next_tile_idx, tmp, batch_idx_in_group); }
    bidb += batch_idx_in_group;
    num_m_blocks = __shfl_sync(0xffffffff, num_m_blocks, batch_idx_in_group);
    if constexpr (Split) {
      num_splits = __shfl_sync(0xffffffff, num_splits, batch_idx_in_group);
    }
    group_start_tile += (batch_idx_in_group == 0
                             ? 0
                             : __shfl_sync(0xffffffff, num_m_blocks_cumulative,
    ...
    if constexpr (LPT) {
      if (!Split || num_splits == 1) {
        ...
        // int num_n_blocks = params.num_n_blocks_ptr ?
        // params.num_n_blocks_ptr[bidb] : num_m_blocks; auto find_log2_floor =
        // [&](int n) { return 31 - cutlass::clz(n); }; int nheads_in_l2 =
        // params.max_kvblocks_in_l2 < num_n_blocks
        //     ? 1 : 1 << find_log2_floor(params.max_kvblocks_in_l2 /
        //     num_n_blocks);
        ...
        int nheads_in_this_section =
            nheads_in_l2 <= nheads_remainder ? nheads_in_l2 : nheads_remainder;
        ...
        if constexpr (Split) {
          ...
          uint32_t bidh_packed =
              reinterpret_cast<uint32_t &>(bidh) +
              (reinterpret_cast<uint32_t &>(num_splits) << 24);
        ...
        //     uint32_t bidh_packed = reinterpret_cast<uint32_t&>(bidh) +
        //     (reinterpret_cast<uint32_t&>(split_idx) << 16) +
        //     (reinterpret_cast<uint32_t&>(num_splits) << 24); bidh =
        //     reinterpret_cast<int&>(bidh_packed);
        // }
        ...
        if constexpr (Split) {
          ...
          uint32_t bidh_packed =
              reinterpret_cast<uint32_t &>(bidh_actual) +
              (reinterpret_cast<uint32_t &>(split_idx) << 16) +
              (reinterpret_cast<uint32_t &>(num_splits) << 24);
          bidh = reinterpret_cast<int &>(bidh_packed);
        }
      ...
      if constexpr (Split) {
        ...
        // Use the top 8 bits to store num_splits and the next 8 bits to store
        // split_idx reinterpret_cast to uint32_t to make sure we're not doing
        // sign extension when we shift
        uint32_t bidh_packed = reinterpret_cast<uint32_t &>(bidh_actual) +
                               ...
                               (reinterpret_cast<uint32_t &>(num_splits) << 24);
        // if (threadIdx.x == 0) {
        //     printf("blockIdx.x = %d, group_start_tiled = %d, bidb = %d,
        //     batch_idx_in_group = %d, mh_block = %d, num_m_blocks = %d, bidh =
        //     %d, bidh_actual = %d, split_idx = %d, num_splits = %d,
        //     bidh_packed = %d\n", blockIdx.x, group_start_tile, bidb,
        //     batch_idx_in_group, mh_block, num_m_blocks, bidh, bidh_actual,
        //     split_idx, num_splits, bidh_packed);
        // }
        bidh = reinterpret_cast<int &>(bidh_packed);
      }
      // if (blockIdx.x <= 9 && threadIdx.x == 0) {
      //     printf("Before returning, blockIdx.x = %d, threadIdx.x = %d,
      //     group_start_tile = %d, batch_idx_in_group = %d, bidb = %d,
      //     num_m_blocks = %d, next_tile_idx = %d, group_end_tile = %d,
      //     m_blocks_in_group = %d, mh_block = %d, bidh = %d, block = %d\n",
      //     blockIdx.x, threadIdx.x, group_start_tile, batch_idx_in_group,
      //     bidb, num_m_blocks, next_tile_idx, group_end_tile,
      //     m_blocks_in_group, mh_block, bidh, block);
      // }
    }
  ...
  CUTLASS_DEVICE WorkTileInfo get_initial_work(Params const &params) const {
    if constexpr (IsProducerWarp) {
      ...
      if (threadIdx.x % cutlass::NumThreadsPerWarp == 0) {
        *work_info_smem = make_int4(work_info.tile_idx, work_info.block,
  ...
  get_next_work(Params const &params, WorkTileInfo const &current_work) const {
    if constexpr (IsProducerWarp) {
      ...
      WorkTileInfo work_info = {
          ...
          current_work.block, current_work.bidh, current_work.bidb};
