...
struct PackGQAManager {
  ...
  using GmemTiledCopyQCpAsync = decltype(make_tiled_copy(
      GmemCopyAtomCpAsync{}, GmemLayoutAtom{},
      Layout<Shape<_1, Int<kGmemElemsPerLoad>>>{})); // Val layout, 8 or 16 vals
                                                     // per load

  // Was trying to have each WG loading Q to the rows in sQ that only that WG
  // needs so that we only need to sync within each WG, but didn't seem to be
  // any faster. using GmemLayoutAtomWG = Layout<Shape <Int<128 /
  // kGmemThreadsPerRow>, Int<NumThreads / 128>, Int<kGmemThreadsPerRow> >,
  //     Stride<Int<kGmemThreadsPerRow>, _128, _1>>;
  // using GmemTiledCopyQCpAsyncWG = decltype(
  //     make_tiled_copy(GmemCopyAtomCpAsync{},
  //                     GmemLayoutAtomNew{},
  //                     Layout<Shape<_1, _1, Int<kGmemElemsPerLoad>>>{}));  //
  //                     Val layout, 8 or 16 vals per load
  ...
  template <int NumThreadsPerRow = kGmemThreadsPerRow, typename Engine,
            typename Layout, typename TensorC>
  ...
  compute_ptr(Tensor<Engine, Layout> &tensor, TensorC const &tRows,
    ...
    static constexpr int NumPtrPerThread =
        cute::ceil_div(CUTE_STATIC_V(cute::size(tRows)), NumThreadsPerRow);
    using TensorType = typename Engine::value_type;
    Tensor tPrPtr =
        make_tensor<TensorType const *>(Shape<Int<NumPtrPerThread>>{});
#pragma unroll
    for (int i = 0; i < NumPtrPerThread; ++i) {
      int const row =
          i * NumThreads + get<0>(tRows(thread_idx % NumThreadsPerRow));
  ...
  load_Q(TensormQ const &mQ, // ((qhead_per_khead, seqlen_q), headdim)
    ...
    auto gmem_thr_copy_Q_cp_async =
        gmem_tiled_copy_Q_cp_async.get_thread_slice(thread_idx);
    Tensor cQ = make_identity_tensor(
        make_shape(size<0>(sQ), size<1>(sQ))); // (BLK_M,BLK_K) -> (blk_m,blk_k)
    Tensor tQcQ = gmem_thr_copy_Q_cp_async.partition_S(
        cQ); // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k)
    Tensor tQsQ = gmem_thr_copy_Q_cp_async.partition_D(
        sQ); // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k)
    // Tensor tQcQ_ = gmem_thr_copy_Q_cp_async.partition_S(cute::flat_divide(cQ,
    // _64{}));       // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k) Tensor tQsQ_ =
    // gmem_thr_copy_Q_cp_async.partition_D(cute::flat_divide(sQ, _64{})); //
    // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k) Tensor tQcQ = group_modes<1,
    // rank(tQcQ_) - 1>(tQcQ_); Tensor tQsQ = group_modes<1, rank(tQsQ_) -
    // 1>(tQsQ_);
    Tensor tQpQ = make_tensor<bool>(make_shape(size<2>(tQsQ)));
#pragma unroll
    for (int k = 0; k < size(tQpQ); ++k) {
      tQpQ(k) = get<1>(tQcQ(_0{}, _0{}, k)) < size<1>(mQ);
    }

    // Similar to loading K and V when PagedKV, it's expensive to compute the
    // pointers for Q. We split the work among threads loading the same row of
    // Q, then __shfl_sync the pointers.
    Tensor mQ_0 = mQ(_, _0{});
...
#pragma unroll
    for (int m = 0; m < size<1>(tQsQ); ++m) {
      ...
      Element const *q_ptr = reinterpret_cast<Element const *>(__shfl_sync(
          0xffffffff,
      ...
      if (idx < seqlen_q * qhead_per_khead) {
        // if (thread_idx == 0) { printf("m: %d, m_idx: %d, h_idx: %d, q_ptr =
        // %p, q_ptr_og = %p\n", m, m_idx, h_idx, q_ptr, &mQ_copy(0,
        // make_coord(h_idx, m_idx), 0));}
        Tensor mQ_cur =
            make_tensor(make_gmem_ptr(q_ptr), Shape<Int<kHeadDim>>{});
        Tensor mQ_cur_copy =
            cute::tiled_divide(mQ_cur, Shape<Int<kGmemElemsPerLoad>>{});
#pragma unroll
        for (int k = 0; k < size<2>(tQsQ); ++k) {
          int ki = get<1>(tQcQ(_0{}, _0{}, k)) / kGmemElemsPerLoad;
          // the "tiled_copy.with(tQpQ(k))"" will fill in zero for columns where
          // tQpQ(k) is false
          // TODO: check this
          cute::copy(gmem_tiled_copy_Q_cp_async.with(tQpQ(k)),
                     mQ_cur_copy(_, ki), tQsQ(_, m, k));
        }
      } // Don't need to fill in 0s for sQ since we're not gonna write the
        // output to gmem for those rows
    }
  };

  template <typename TensormLSE, typename TensorsLSE, typename TiledMma>
  CUTLASS_DEVICE static void store_LSE(
      TensormLSE &mLSE, // ((qhead_per_khead, seqlen_q))
      TensorsLSE const
          &tLSErLSE, // (kBlockM) split across threads according to tiled_mma
      TiledMma tiled_mma, cutlass::FastDivmod const &qhead_per_khead_divmod,
    ...
    Tensor taccOcO_row = make_tensor(
        taccOcO.data(),
        flash::convert_layout_acc_rowcol(taccOcO.layout()))(_, _0{});
    CUTE_STATIC_ASSERT_V(size(tLSErLSE) == size(taccOcO_row)); // MMA_M

    // If PackGQA, we split the work of compute divmod among threads in the same
    // row
    static constexpr int kMmaThreadsPerRow =
        size<0, 0>(typename TiledMma::AtomLayoutC_TV{});
    ...
    Tensor tPrLSEPtr = compute_ptr<kMmaThreadsPerRow>(
        mLSE, taccOcO_row, qhead_per_khead_divmod, thread_idx, m_block);
...
#pragma unroll
    for (int mi = 0; mi < size(tLSErLSE); ++mi) {
      ...
      float *ptr_LSE_cur = reinterpret_cast<float *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrLSEPtr[0]),
                      mi % kMmaThreadsPerRow, kMmaThreadsPerRow));
  ...
  store_O(TensormO &mO,         // ((qhead_per_khead, seqlen_o), headdim)
          TensorrO const &tOrO, // (kBlockM, kHeadDim) split across threads
                                // according to gmem_tiled_copy_O
          cutlass::FastDivmod const &qhead_per_khead_divmod,
          int const thread_idx, int const seqlen_o, int const m_block) {
...
#pragma unroll
    for (int k = 0; k < size(tOpO); ++k) {
      tOpO(k) = get<1>(tOcO(_0{}, _0{}, k)) < size<1>(mO);
    }

    // Similar to loading K and V when PagedKV, it's expensive to compute the
    // pointers for O. We split the work among threads loading the same row of
    // O, then __shfl_sync the pointers.
    Tensor mO_0 = mO(_, _0{});
...
#pragma unroll
    for (int m = 0; m < size<1>(tOrO); ++m) {
      ...
      Element *o_ptr = reinterpret_cast<Element *>(__shfl_sync(
          0xffffffff,
          reinterpret_cast<uint64_t>(tPrOPtr(m / kGmemThreadsPerRow)),
          m % kGmemThreadsPerRow, kGmemThreadsPerRow));
      if (idx < seqlen_o * qhead_per_khead) {
        Tensor mO_cur =
            make_tensor(make_gmem_ptr(o_ptr), Shape<Int<kHeadDim>>{});
        Tensor mO_cur_copy =
            cute::tiled_divide(mO_cur, Shape<Int<kGmemElemsPerStore>>{});
#pragma unroll
  ...
  store_O_direct(TensormO &mO,         // ((qhead_per_khead, seqlen_o), headdim)
                 TensorrO const &tOrO, // (kBlockM, kHeadDim) split across
                                       // threads according to tiled_mma
                 TiledMma tiled_mma,
                 cutlass::FastDivmod const &qhead_per_khead_divmod,
                 int const thread_idx, int const seqlen_o, int const m_block) {
    ...
    cute::Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, Element>
        gmem_copy_direct;
    // Reshape acc from ((2, 2, V), MMA_M, MMA_N) to (nrow=(2, MMA_M), ncol=(2,
    // V, MMA_N))
    ...
    // O, then __shfl_sync the pointers.
    Tensor mO_0 = mO(_, _0{});
    Tensor tPrOPtr = compute_ptr<kMmaThreadsPerRow>(
        mO_0, taccOcO_row, qhead_per_khead_divmod, thread_idx, m_block);
...
#pragma unroll
    for (int m = 0; m < size<1>(tOrO_copy); ++m) {
      ...
      Element *o_ptr = reinterpret_cast<Element *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrOPtr[0]),
                      m % kMmaThreadsPerRow, kMmaThreadsPerRow));
      if (row < seqlen_o * qhead_per_khead) {
        Tensor mO_cur =
            make_tensor(make_gmem_ptr(o_ptr), Shape<Int<kHeadDim>>{});
        Tensor mO_cur_copy =
            cute::tiled_divide(mO_cur, Shape<Int<kGmemElemsPerStoreDirect>>{});
#pragma unroll
        for (int k = 0; k < size<2>(tOrO_copy); ++k) {
          ...
          if (col < size<1>(mO)) {
            cute::copy(gmem_copy_direct, tOrO_copy(_, m, k),
                       mO_cur_copy(_, col / kGmemElemsPerStoreDirect));
          }
        }
      }
    }
