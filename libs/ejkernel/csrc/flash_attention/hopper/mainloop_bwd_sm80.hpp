...
template <int Stages, int Stages_dO, class TileShape_MNK_, class Element_,
          class ElementAccum_, class ArchTag_, bool Is_causal_, bool Is_local_,
          bool Has_softcap_, bool Varlen_, bool Deterministic, bool SdP_swapAB_,
          bool dKV_swapAB_, bool dQ_swapAB_, int NumMmaWarpGroups = 2,
          int AtomLayoutMSdP = 1, int AtomLayoutNdKV = 8, int AtomLayoutMdQ = 1,
...
struct CollectiveMainloopBwdSm80 {
  ...
  using BlockMN_t =
      flash::BlockMN<SeqlenInfo_t, kBlockM, kBlockN, Is_causal, Is_local>;
  ...
  static constexpr bool Mma_dKV_is_RS = AtomLayoutMSdP == 1 &&
                                        AtomLayoutNdKV == NumMmaWarps &&
                                        SdP_swapAB && !dKV_swapAB;
  static constexpr bool Mma_dQ_is_RS =
      AtomLayoutMSdP == NumMmaWarps && AtomLayoutMdQ == NumMmaWarps &&
      !SdP_swapAB && !dQ_swapAB; // If dQ_swapAB we can't use RS
  ...
  using TiledMmaSdP =
      TiledMMA<MMA_Atom_Arch, AtomLayoutSdP,
               Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutSdP{}))>,
                    Int<(MmaSdPEvenN ? 16 : 8) *
                        CUTE_STATIC_V(size<1>(AtomLayoutSdP{}))>,
                    _16>>;
  ...
  using TiledMmadKV =
      TiledMMA<MMA_Atom_Arch, AtomLayoutdKV,
               Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutdKV{}))>,
                    Int<(MmadKVEvenN ? 16 : 8) *
                        CUTE_STATIC_V(size<1>(AtomLayoutdKV{}))>,
                    _16>>;
  ...
  using TiledMmadQ = TiledMMA<
      MMA_Atom_Arch, AtomLayoutdQ,
      Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutdQ{}))>,
  ...
  using SmemLayoutAtomKV = decltype(composition(
      ...
      Layout<Shape<_8, Int<kBlockKGmem>>, Stride<Int<kBlockKGmem>, _1>>{}));
  using SmemLayoutK = decltype(tile_to_shape(SmemLayoutAtomKV{},
                                             select<1, 2>(TileShape_MNK{})));

  using SmemLayoutV = decltype(tile_to_shape(SmemLayoutAtomKV{},
                                             select<1, 2>(TileShape_MNK{})));
  ...
  // We set stride to be multiple of 64 so that if ShuffleLSE, even if threads
  // read from sLSE but out of bounds, it's still a valid smem address.
  using SmemLayoutLSE =
  ...
  using SmemLayoutLSEMma = std::conditional_t<
      ...
      cute::Layout<cute::Shape<Int<kBlockM>, Int<kBlockN>, Int<kStages>>,
  ...
  using SmemLayoutPdSt = decltype(cute::composition(
      SmemLayoutPdS{}, make_layout(make_shape(Int<kBlockN>{}, Int<kBlockM>{}),
  ...
  using GmemTiledCopyLSE = decltype(make_tiled_copy(
      GmemCopyAtomLSE{}, GmemLayoutAtomLSE{},
  ...
  // needs to keep statistic for 2 rows. static constexpr bool ShuffleLSE =
  // SdP_swapAB && kHeadDim <= 64; static constexpr bool ShuffledPsum =
  // SdP_swapAB && kHeadDim <= 64;
  ...
  using SmemP_t = std::conditional_t<
      Mma_dKV_is_RS, cute::array<Element, 0>,
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    // to params.softmax_scale_log2). In the backward, we need to multiply by (1
    // - tanh^2) * softmax_scale / softcap_val * softcap_val = (1 - tanh^2) *
    // softmax_scale. Instead we multiply by (1 - tanh^2) and multiply dK and dV
    // by params.softmax_scale (the original softmax_scale) at the end.
    return {args.ptr_Q,
            args.shape_Q,
            args.stride_Q,
            args.ptr_K,
  ...
  CUTLASS_DEVICE bool mma(Params const &params, FrgTensordKV &tdKrdK,
                          FrgTensordKV &tdVrdV, int thread_idx,
    ...
    SeqlenInfo_t seqlen_info{bidb,
                             get<0>(params.shape_Q),
                             size<0>(params.shape_K),
                             params.cu_seqlens_q,
                             params.cu_seqlens_k,
                             params.seqused_q,
                             params.seqused_k};
    ...
    Tensor mQ =
        make_tensor(make_gmem_ptr(params.ptr_Q), params.shape_Q,
                    params.stride_Q)(_, _, bidh, !is_varlen_q ? bidb : 0);
    Tensor mdO =
        make_tensor(make_gmem_ptr(params.ptr_dO), params.shape_dO,
                    params.stride_dO)(_, _, bidh, !is_varlen_q ? bidb : 0);
    Tensor mK =
        make_tensor(make_gmem_ptr(params.ptr_K), params.shape_K,
                    params.stride_K)(_, _, bidh_kv, !is_varlen_k ? bidb : 0);
    Tensor mV =
        make_tensor(make_gmem_ptr(params.ptr_V), params.shape_V,
                    params.stride_V)(_, _, bidh_kv, !is_varlen_k ? bidb : 0);
    Tensor mLSE =
        make_tensor(make_gmem_ptr(params.ptr_LSE_log2), params.shape_LSE,
                    params.stride_LSE_log2)(_, bidh, !is_varlen_q ? bidb : 0);
    Tensor mdPsum =
        make_tensor(make_gmem_ptr(params.ptr_dPsum), params.shape_LSE,
                    params.stride_dPsum)(_, bidh, !is_varlen_q ? bidb : 0);
    Tensor mdQaccum = make_tensor(
        make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.ptr_dQaccum)),
        params.shape_dQaccum,
        params.stride_dQaccum)(_, bidh, !is_varlen_q ? bidb : 0);
    ...
    Tensor gK = local_tile(
        ...
        select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{})); // (N, K)
    Tensor gV = local_tile(
        domain_offset(make_coord(seqlen_info.offset_k, _0{}), mV),
        select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{})); // (N, K)
    Tensor gLSE =
        local_tile(domain_offset(make_coord(seqlen_info.offset_q_padded), mLSE),
    ...
    Tensor gdQaccum = local_tile(
        domain_offset(make_coord(seqlen_info.offset_q_padded * kHeadDim),
                      mdQaccum),
        Shape<Int<kBlockM * kHeadDim>>{}, make_coord(_)); // (M * K, _)
    ...
    auto smem_tiled_copy_dS = cute::conditional_return<!dQ_swapAB>(
        ...
        make_tiled_copy_B(cute::conditional_return<MmadQEvenN>(
                              SmemCopyAtom{}, SmemCopyAtomHalf{}),
                          tiled_mma_dQ));
    ...
    auto smem_tiled_copy_Kt = cute::conditional_return<!dQ_swapAB>(
        make_tiled_copy_B(
            cute::conditional_return<MmadQEvenN>(SmemCopyAtomTransposed{},
                                                 SmemCopyAtomTransposedHalf{}),
            tiled_mma_dQ),
    ...
    // thr_mma_SdP.partition_C(sLSEMma) has shape (MMA=4, MMA_M, MMA_N, PIPE),
    // we only take the col indices or row indices, depending on whether
    // SdP_swapAB.
    Tensor tSsLSEMma =
        logical_divide(thr_mma_SdP.partition_C(sLSEMma),
                       Shape<_2>{}); // (2, 2, MMA_M, MMA_N, PIPE)
    ...
    Tensor tSsdPsumMma =
        logical_divide(thr_mma_SdP.partition_C(sdPsumMma), Shape<_2>{});
    ...
    static constexpr int kStatsPerThread =
        cute::ceil_div(decltype(size(tSsLSE))::value, 8);
...
#pragma unroll
    for (int k = 0; k < size(tdOpdO); ++k) {
      tdOpdO(k) = get<1>(tQcQ(_0{}, _0{}, k)) < get<1>(params.shape_dO);
    }

    int const seqlen_q = seqlen_info.seqlen_q;
    int const seqlen_k = seqlen_info.seqlen_k;

    flash::Mask<kBlockM, kBlockN, false /*PackGQA*/, TiledMmaSdP, SdP_swapAB>
        mask(thread_idx, seqlen_q, seqlen_k, params.window_size_left,
             params.window_size_right, 0 /*sink_token_length*/,
...
#pragma unroll
      for (int k = 0; k < size(tKpK); ++k) {
        tKpK(k) = get<1>(tKVcKV(_0{}, _0{}, k)) < get<1>(params.shape_K);
      }
#pragma unroll
      for (int k = 0; k < size(tVpV); ++k) {
        tVpV(k) = get<1>(tKVcKV(_0{}, _0{}, k)) < get<1>(params.shape_V);
      }
      ...
      int const seqlenk_row_limit =
          seqlen_k - n_block * kBlockN - get<0>(tKVcKV(_0{}, _0{}, _0{}));
#pragma unroll
...
#pragma unroll
          for (int k = 0; k < size<2>(tVsV); ++k) {
            cute::copy(gmem_tiled_copy_QKV.with(tVpV(k) && predicate_n),
                       tVgV(_, m, k), tVsV(_, m, k));
          }
        }
      }
...
#pragma unroll
          for (int k = 0; k < size<2>(tKsK); ++k) {
            cute::copy(gmem_tiled_copy_QKV.with(tKpK(k) && predicate_n),
                       tKgK(_, m, k), tKsK(_, m, k));
          }
        }
      }
      flash::cp_async_fence();
    ...
    auto load_Q_LSE = [&](int const m_block, int const smem_pipe_write) {
      // if (cute::thread0()) { printf("Inside load_Q_LSE, m_block = %d,
      ...
      // entries of t0QcQ are known at compile time. int const seqlenq_row_limit
      // = -int(get<0>(tQcQ(_0{}, _0{}, _0{}))) + (EvenM
      //     ? seqlen_info.seqlen_q - m_block * kBlockM
      //     : std::min(seqlen_info.seqlen_q - m_block * kBlockM, kBlockM));
      // Need Clear_OOB_MN to be true here since the gemm will sum over the
      // kBlockM dimension flash::copy</*Is_even_MN=*/false,
      // /*Is_even_K=*/false, /*Clear_OOB_MN=*/true, /*Clear_OOB_K=*/true>(
      //     gmem_tiled_copy_QKV, tQgQ(_, _, _, m_block), tQsQ_cur, t0QcQ, tQpQ,
      //     seqlenq_row_limit);
      int const seqlenq_row_limit = seqlen_info.seqlen_q - m_block * kBlockM -
                                    get<0>(tQcQ(_0{}, _0{}, _0{}));
#pragma unroll
...
#pragma unroll
          for (int k = 0; k < size<2>(tQsQ); ++k) {
            cute::copy(gmem_tiled_copy_QKV.with(tQpQ(k) && predicate_m),
                       tQgQ_cur(_, m, k), tQsQ_cur(_, m, k));
          }
        }
      }
...
// elements in sLSE are filled. Without this we might have uninitialized sLSE
// values.
#pragma unroll
      for (int m = 0; m < size<1>(tLSEsLSE); ++m) {
        if (get<0>(tLSEcLSE(_0{}, m)) < kBlockM) {
          cute::copy(gmem_tiled_copy_lse, tLSEgLSE_cur(_, m),
                     tLSEsLSE_cur(_, m));
        }
      }
    };
    ...
    auto load_dO_dPsum = [&](int const m_block, int const smem_pipe_write) {
      // if (cute::thread0()) { printf("Inside load_dO_dPsum, m_block = %d,
      // smem_pipe_write = %d\n", m_block, smem_pipe_write); }
      Tensor tdOsdO_cur = tdOsdO(_, _, _, smem_pipe_write);
      Tensor tdOgdO_cur = tdOgdO(_, _, _, m_block);
      // int const seqlenq_row_limit = -int(get<0>(tQcQ(_0{}, _0{}, _0{}))) +
      // (EvenM
      //     ? seqlen_info.seqlen_q - m_block * kBlockM
      //     : std::min(seqlen_info.seqlen_q - m_block * kBlockM, kBlockM));
      // flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
      // /*Clear_OOB_MN=*/true, /*Clear_OOB_K=*/true>(
      //     gmem_tiled_copy_QKV, tdOgdO(_, _, _, m_block), tdOsdO_cur, t0QcQ,
      //     tQpQ, seqlenq_row_limit);
      int const seqlenq_row_limit = seqlen_info.seqlen_q - m_block * kBlockM -
                                    get<0>(tQcQ(_0{}, _0{}, _0{}));
#pragma unroll
...
#pragma unroll
          for (int k = 0; k < size<2>(tdOsdO); ++k) {
            cute::copy(gmem_tiled_copy_QKV.with(tdOpdO(k) && predicate_m),
                       tdOgdO_cur(_, m, k), tdOsdO_cur(_, m, k));
          }
        }
      }
...
#pragma unroll
      for (int m = 0; m < size<1>(tLSEsdPsum); ++m) {
        if (get<0>(tLSEcLSE(_0{}, m)) < kBlockM) {
          cute::copy(gmem_tiled_copy_lse, tLSEgdPsum_cur(_, m),
                     tLSEsdPsum_cur(_, m));
        }
      }
    };
    ...
    auto load_Q_next = [&] {
      // if (cute::thread0()) { printf("m_block = %d, m_block_max = %d,
      // smem_pipe_write = %d\n", m_block, m_block_max, smem_pipe_write); }
      if (m_block + (kStages > 1 ? kStages - 1 : 1) < m_block_max) {
        load_Q_LSE(m_block + (kStages > 1 ? kStages - 1 : 1),
    ...
    auto load_dO_next = [&] {
      // int smem_pipe_write_do_cur = Q_dO_same_stages ? smem_pipe_write :
      // smem_pipe_write_do;
      if (m_block + kStages_dO < m_block_max) {
        // load_dO_dPsum(m_block + kStages_dO, kStages_dO > 1 ?
        // smem_pipe_write_do_cur : 0);
        load_dO_dPsum(m_block + kStages_dO,
                      kStages_dO > 1 ? smem_pipe_write_do : 0);
      }
      cute::cp_async_fence();
    };
    ...
    auto bwd_step = [&](int m_block, auto mask_fn) {
      Tensor tSrS =
          partition_fragment_C(tiled_mma_SdP, select < !SdP_swapAB ? 0 : 1,
      ...
      flash::gemm_sm80<false /*A_in_regs*/, false /*B_in_regs*/, SdP_swapAB>(
          tSrS, tSrQ, tSrK, tSsQ(_, _, _, kStages > 1 ? smem_pipe_read : 0),
          tSsK, tiled_mma_SdP, smem_tiled_copy_QdO, smem_tiled_copy_KV,
          smem_thr_copy_QdO, smem_thr_copy_KV, nullptr /*hook*/);
      Tensor tLSErLSE = cute::conditional_return<!ShuffleLSE>(
          make_fragment_like(tSsLSE(_, _0{})),
...
#pragma unroll
        for (int i = 0; i < kStatsPerThread; ++i) {
          ...
          tLSErLSE(i) = tSsLSE((thread_idx % 32) / 4 + i * 8,
                               kStages > 1 ? smem_pipe_read : 0);
        }
      }
      ...
      Tensor scores = make_tensor(
          ...
          flash::convert_layout_acc_rowcol</*Transposed=*/SdP_swapAB>(
              tSrS.layout()));
      // dtanh needs to happen before masking, otherwise we get 1 - (-inf)^2 =
      // NaN in the dtanh if (cute::thread0()) { print_tensor(scores); }
      auto dtanh = [&] {
        if constexpr (Has_softcap)
      ...
      for (int mi = 0; mi < size<0>(scores); ++mi) {
        float const lse_scaled = [&] {
          if constexpr (!ShuffleLSE)
            ...
            return __shfl_sync(0xffffffff, tLSErLSE(mi / 8),
                               (mi % 8) * 4 + (thread_idx % 4));
        }();
#pragma unroll
        for (int ni = 0; ni < size<1>(scores); ++ni) {
          scores(mi, ni) =
              exp2f(scores(mi, ni) * params.softmax_scale_log2 - lse_scaled);
        }
      }
      ...
      flash::gemm_sm80<false /*A_in_regs*/, V_in_regs, SdP_swapAB>(
          tdPrdP, tdPrdO, tdPrV_cur,
          tdPsdO(_, _, _, kStages_dO > 1 ? smem_pipe_read_do_cur : 0), tdPsV,
      ...
      Tensor tLSErdPsum = cute::conditional_return<!ShuffledPsum>(
          make_fragment_like(tSsdPsum(_, _0{})),
          make_tensor<ElementAccum>(Int<kStatsPerThread>{}));
      if constexpr (!ShuffledPsum) {
        cute::copy(tSsdPsum(_, kStages_dO > 1 ? smem_pipe_read_do_cur : 0),
                   tLSErdPsum);
      } else {
#pragma unroll
        for (int i = 0; i < kStatsPerThread; ++i) {
          tLSErdPsum(i) = tSsdPsum((thread_idx % 32) / 4 + i * 8,
                                   kStages_dO > 1 ? smem_pipe_read_do_cur : 0);
        }
      }
      ...
      for (int mi = 0; mi < size<0>(dS); ++mi) {
        float const dP_sum_cur = [&] {
          if constexpr (!ShuffledPsum)
            ...
            return __shfl_sync(0xffffffff, tLSErdPsum(mi / 8),
                               (mi % 8) * 4 + (thread_idx % 4));
        }();
#pragma unroll
      ...
      if constexpr (Mma_dKV_is_RS) {
        ...
        flash::gemm_rs_sm80(tdVrdV, tdVrP, tdVrdO, tdVsdO_cur, tiled_mma_dKV,
                            smem_tiled_copy_QdOt, smem_thr_copy_QdOt);
      } else {
        Tensor tdVrP =
            mma_partition_fragment_AB</*A=*/!dKV_swapAB>(thr_mma_dKV, sPt);
        flash::gemm_sm80<false /*A_in_regs*/, false /*B_in_regs*/,
                         /*SwapAB=*/dKV_swapAB>(
            tdVrdV, tdVrP, tdVrdO, tdVsPt, tdVsdO_cur, tiled_mma_dKV,
            smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt,
            smem_thr_copy_QdOt, nullptr);
      }
      ...
      auto do_mma_dQ = [&](auto hook) {
        ...
        flash::gemm_sm80<false /*A_in_regs*/, false /*B_in_regs*/,
            ...
            // smem_thr_copy_Kt, load_dO_next);
            smem_tiled_copy_dS, smem_tiled_copy_Kt, smem_thr_copy_dS,
            smem_thr_copy_Kt, hook);
        ...
        static_assert(CUTE_STATIC_V(size(tdQrdQ_atomic)) ==
...
#pragma unroll
      ...
      if constexpr (Mma_dKV_is_RS) {
        ...
        flash::gemm_rs_sm80(tdKrdK, tdKrdS, tdKrQ, tdKsQ_cur, tiled_mma_dKV,
                            smem_tiled_copy_QdOt, smem_thr_copy_QdOt);
      } else {
        Tensor tdKrdS =
        ...
        flash::gemm_sm80<false /*A_in_regs*/, false /*B_in_regs*/,
                         /*SwapAB=*/dKV_swapAB>(
            tdKrdK, tdKrdS, tdKrQ, tdKsdSt, tdKsQ_cur, tiled_mma_dKV,
            smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt,
            smem_thr_copy_QdOt,
    ...
    if constexpr ((Is_causal || Is_local) && SeparateMaskingIterations) {
      ...
      int const m_block_masking_max = ((n_block + 1) * kBlockN - 1 + seqlen_q -
                                       seqlen_k - params.window_size_right) /
                                          kBlockM +
                                      1;
    ...
    int const m_block_max_before_local_mask =
        !Is_local || !SeparateMaskingIterations
            ? m_block_max
            : std::min(m_block_max, (n_block * kBlockN + seqlen_q - seqlen_k +
                                     params.window_size_left) /
                                        kBlockM);

    auto mask_fn = [&](auto &tSrS, int m_block) {
      mask.template apply<true /*Seqlenk_mask*/,
                          Is_causal && !SeparateMaskingIterations,
                          Is_local && !SeparateMaskingIterations>(tSrS, m_block,
                                                                  n_block);
    };
    CUTLASS_PRAGMA_NO_UNROLL
    ...
    if constexpr (Is_local && SeparateMaskingIterations) {
      auto mask_fn = [&](auto &tSrS, int m_block) {
        mask.template apply<true /*Seqlenk_mask*/, false /*Causal_mask*/,
