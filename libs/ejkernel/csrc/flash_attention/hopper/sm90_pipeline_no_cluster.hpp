...
// As of Cutlass v3.6.0, if size(ClusterShape) == 1, PipelineTmaAsync has all
// threads signaling the barrier during consumer_release. This causes a perf
// regression in FA3 forward pass (especially hdim 128 causal). We instead
// reimplement the version of PipelineTmaAsync before v3.6.0 where only 1 out of
// 128 threads signals the barrier.
//
...
public:
  ...
  static CUTLASS_DEVICE void init_barriers(SharedStorage &storage,
    ...
    if (is_initializing_warp) {
      ...
      uint32_t const num_consumer_warpgroups_per_cluster =
          (params.num_consumers + NumThreadsPerWarpGroup - 1) /
      ...
      cutlass::arch::detail::initialize_barrier_array_pair_aligned<
          decltype(storage.full_barrier_), decltype(storage.empty_barrier_),
          Stages>(storage.full_barrier_, storage.empty_barrier_,
  ...
  CUTLASS_DEVICE PipelineTmaAsyncNoCluster(SharedStorage &storage,
                                           Params params,
                                           ClusterShape cluster_shape,
                                           InitBarriers = {}, InitMasks = {})
      : Base(storage, params, make_shape(_1{}, _1{}, _1{}) /*cluster_shape*/,
             cute::false_type{} /*init_barriers*/,
             cute::false_type{} /*init_masks*/),
  ...
  CUTLASS_DEVICE PipelineTmaAsyncNoCluster(SharedStorage &storage,
                                           Params params,
                                           ClusterShape cluster_shape)
      : PipelineTmaAsyncNoCluster(storage, params, cluster_shape,
                                  cute::true_type{}, cute::true_type{}) {}

  template <class ClusterShape, class InitBarriers>
  CUTLASS_DEVICE
  PipelineTmaAsyncNoCluster(SharedStorage &storage, Params params,
...
private:
  ...
  void consumer_release(uint32_t stage, uint32_t skip = false) {
    empty_barrier_ptr_[stage].arrive(
        0 /*dst_blockid_*/,
        uint32_t(threadIdx.x % cutlass::NumThreadsPerWarpGroup == 0) &
            (!skip) /*is_signaling_thread*/);
  }
};
