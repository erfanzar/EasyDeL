...
thread_reduce_(Tensor<Engine0, Layout0> const &tensor,
...
#pragma unroll
    for (int mi = 0; mi < size<0>(tensor); mi++) {
      summary(mi) = zero_init && ni == 0 ? tensor(mi, ni)
...
template <bool zero_init = true, bool warp_reduce = true, typename Engine0,
...
scale_apply_exp2(Tensor<Engine0, Layout0> &tensor,
  ...
  // For FP8, we can subtract max by 8.0 so that the value after exp2 is in the
  // range of [0, 256]. This lets us use more of the FP8 range (instead of just
  // [0, 1]) to reduce underflow.
  static constexpr float max_offset =
      float(Max_offset); // We can only template on int, not float
  static_assert(Layout0::rank == 2, "Only support 2D Tensor");
  static_assert(Layout1::rank == 1, "Only support 1D Tensor");
...
template <int kNRows, int Max_offset = 0> struct Softmax {
...
#pragma unroll
  ...
  __forceinline__ __device__ void online_softmax(Tensor0 &acc_s) {
    ...
    // We don't do the reduce across threads here since we don't need to use the
    // row_sum. We do that reduce at the end when we need to normalize the
    // softmax.
    flash::reduce_sum</*zero_init=*/Is_first, /*warp_reduce=*/false>(scores,
                                                                     row_sum);
  };
...
#pragma unroll
    for (int mi = 0; mi < size(row_sum); ++mi) {
      ...
      // For FP8, we might have scaled the output of exp by 2**8 so we need to
      // divide sum by that amount.
