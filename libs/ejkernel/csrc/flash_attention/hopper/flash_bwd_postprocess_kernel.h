...
template <class TileShape_MK_, class Element, class ElementAccum,
          class ArchTag_, int kNThreads, class TiledMma, bool dQ_swapAB>
...
public:
  ...
  static_assert(!IsSm90 || kNThreads % cutlass::NumThreadsPerWarpGroup == 0,
  ...
  using R2SLayoutAtomdQaccum =
      std::conditional_t<IsSm90,
                         Layout<Shape<Int<cutlass::NumThreadsPerWarpGroup>,
  ...
  using SmemLayoutdQaccum = std::conditional_t<
      IsSm90,
      Layout<Shape<Int<kBlockM * kHeadDim / NumdQWarpGgroups>,
                   Int<NumdQWarpGgroups>>>,
  ...
  using SmemCopyAtomdQ = Copy_Atom<
      std::conditional_t<IsSm90,
                         std::conditional_t<!dQ_swapAB, cute::SM90_U32x4_STSM_N,
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    return {args.ptr_dQaccum,   args.shape_dQaccum, args.stride_dQaccum,
            args.ptr_dQ,        args.shape_dQ,      args.stride_dQ,
            args.softmax_scale, args.cu_seqlens,    args.seqused};
  }
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    flash::SeqlenInfo<true /*Varlen*/, kBlockM> seqlen_info(
        bidb, size<0>(params.shape_dQ), params.cu_seqlens, params.seqused);
    bool const is_varlen = params.cu_seqlens;
    if (is_varlen && m_block * kBlockM >= seqlen_info.seqlen) {
      return;
    }

    // Step 1: load dQaccum from gmem to smem
    Tensor mdQaccum =
        make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum const *>(
                        params.ptr_dQaccum)),
    ...
    Tensor gdQaccum = local_tile(
        domain_offset(make_coord(seqlen_info.offset_padded * kHeadDim),
                      mdQaccum),
        Shape<Int<kBlockM * kHeadDim>>{}, make_coord(m_block)); // (M * K)
    if constexpr (IsSm90) {                                     // Use BulkCopy
      static constexpr uint32_t TmaTransactionBytesdQaccum =
          static_cast<uint32_t>(size(SmemLayoutdQaccumFlat{}) *
      ...
      if (thread_idx == 0) {
        ...
        copy(bulk_copy.with(*reinterpret_cast<uint64_t *>(
                 &shared_storage.barrier_dQaccum)),
             gdQaccum, sdQaccum_flat);
      }
      __syncthreads();
...
#pragma unroll
    ...
    Tensor gdQ =
        local_tile(domain_offset(make_coord(seqlen_info.offset, _0{}), mdQ),
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/EvenM, /*Is_even_K=*/true,
                /*Clear_OOB_MN=*/false>(gmem_tiled_copy_dQ, tdQsdQ, tdQrdQ,
                                        tdQcdQ, tdQpdQ, kBlockM);
