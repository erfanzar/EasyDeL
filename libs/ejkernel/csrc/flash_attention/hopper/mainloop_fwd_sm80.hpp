...
template <int kNWarps, int Stages, bool Q_in_regs, class TileShape_MNK_,
          ...
          bool PagedKV_, bool AppendKV_, bool PackGQA_, bool Split_>
struct CollectiveMainloopFwdSm80 {
  ...
  using MMA_Atom_Arch = std::conditional_t<
      ...
      std::conditional_t<std::is_same_v<Element, cutlass::half_t>,
  ...
  using SmemLayoutVt = decltype(composition(
      SmemLayoutV{},
      make_ordered_layout(make_shape(shape<2>(TileShape_MNK{}),
  ...
  static_assert(NumMmaThreads % kGmemThreadsPerRowAppend == 0,
  ...
  static_assert(cutlass::NumThreadsPerWarp % kGmemThreadsPerRowAppend == 0,
                "kGmemThreadsPerRowAppend must divide NumThreadsPerWarp");
  using GmemLayoutAtomAppend =
      Layout<Shape<Int<NumMmaThreads / kGmemThreadsPerRowAppend>,
  ...
  static_assert(
      !AppendKV ||
          kBlockM % CUTE_STATIC_V(shape<0>(GmemLayoutAtomAppend{})) == 0,
      "kBlockM must be a multiple of NumMmaThreads / kGmemThreadsPerRowAppend");
  ...
  using TensorStorage = std::conditional_t<Share_QV_Smem, TensorStorageSharedQV,
  ...
  CUTLASS_DEVICE bool
  mma(Params const &params, FrgTensorO &tOrO, Softmax &softmax,
    ...
    Tensor mK =
        make_tensor(make_gmem_ptr(params.ptr_K + seqlen_info.offset_k *
                                                     get<0>(params.stride_K)),
                    params.shape_K,
                    params.stride_K)(_, _, bidh_kv, !is_varlen_k ? bidb_kv : 0);
    Tensor gK = local_tile(mK, select<1, 2>(TileShape_MNK{}),
                           make_coord(_, _0{})); // (N, K, _)
    Tensor mV =
        make_tensor(make_gmem_ptr(params.ptr_V + seqlen_info.offset_k *
                                                     get<0>(params.stride_V)),
                    params.shape_K,
...
#pragma unroll
      ...
      // from the limit (seqlen_q - m_block * kBlockM). This is because the
      ...
      using PackGQAt = flash::PackGQAManager<get<0>(TileShape_MNK{}),
                                             get<2>(TileShape_MNK{}),
                                             NumMmaThreads, Element>;
      PackGQAt::load_Q(mQ, sQ, params.qhead_per_khead_divmod, thread_idx,
                       seqlen_q, m_block);
    }
    cute::cp_async_fence();
    ...
    PagedKVManager_t paged_kv_manager(
        ...
        params.page_size_divmod /*blockN_per_page_size_divmod, not used since we
                                   don't use TMA*/
        ,
        bidb_kv, bidh_kv, thread_idx, seqlen_info.seqlen_k,
        seqlen_info.leftpad_k,
        0 /*bidb_kv_idx, not used since we don't use TMA for Sm8x*/
    );

    auto load_K = [&](int const n_block, int const smem_pipe_write,
      ...
      if constexpr (!PagedKV) {
        ...
        // offset from the limit (seqlen_k - n_block * kBlockN). This is because
        // the entries of t0KVcKV are known at compile time.
        int const seqlenk_row_limit =
            -int(get<0>(tKVcKV(_0{}, _0{}, _0{}))) +
            (EvenN ? seqlen_info.seqlen_k - n_block * kBlockN
                   : (!Seqlenk_mask
                          ? kBlockN
                          : std::min(seqlen_info.seqlen_k - n_block * kBlockN,
                                     kBlockN)));
        // We don't need to clear the sK smem tiles since we'll mask out the
        // scores anyway.
        flash::copy</*Is_even_MN=*/!Seqlenk_mask && EvenN, /*Is_even_K=*/false,
                    /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true>(
            gmem_tiled_copy_QKV, tKgK(_, _, _, n_block), tKsK_cur, t0KVcKV,
            tKVpKV, seqlenk_row_limit);
      } else {
        paged_kv_manager.template load_page_table<Seqlenk_mask>(n_block);
        paged_kv_manager.template load_K<Seqlenk_mask>(
            n_block, sK(_, _, smem_pipe_write));
      }
    };

    auto load_V = [&](int const n_block, int const smem_pipe_write,
                      auto need_seqlenk_masking_type) {
      static constexpr bool Seqlenk_mask =
          decltype(need_seqlenk_masking_type)::value;
      if constexpr (!PagedKV) {
...
#pragma unroll
            for (int k = 0; k < size<2>(tVsV); ++k) {
              cute::copy(gmem_tiled_copy_QKV.with(tKVpKV(k) && predicate_n),
                         tVgV_cur(_, m, k), tVsV_cur(_, m, k));
            }
          }
        }
      } else {
        paged_kv_manager.template load_V<Seqlenk_mask>(
            n_block, sV(_, _, smem_pipe_write));
      }
    };
    ...
    auto preprocess_Q = [&] {
      if constexpr (!AppendKV) {
        ...
        if (get<1>(params.shape_rotary) > 0) { // Apply rotary to Q
          using Rotary_t = Rotary<kBlockM, kHeadDim, NumMmaThreads, Element,
          ...
          if (params.is_rotary_interleaved) {
            ...
            flash::cp_async_wait<Share_QV_Smem ? 1 : kStages * 2 - 1>();
            __syncthreads();
            rotary.apply_Q_interleaved(sQ, tRrCos, tRrSin, m_block,
                                       qhead_per_khead);
          } else {
            ...
            flash::cp_async_wait<Share_QV_Smem ? 1 : kStages * 2 - 1>();
            __syncthreads();
            rotary.apply_Q_contiguous(sQ, tRrCosCont, tRrSinCont, m_block,
                                      qhead_per_khead);
          }
        } else {
          flash::cp_async_wait<Share_QV_Smem ? 1 : kStages * 2 - 1>();
    ...
    // If Share_QV_Smem, we load Q, then load 1 stage of K, then (optionally)
    // rotate Q and read from smem_q to registers, then load V. If !Share_QV,
    // Smem, we load Q, load all stages of K & V, then (optionally) rotate Q.

    if constexpr (Share_QV_Smem) {
      load_K(n_block, 0, cute::true_type{} /*Seqlenk_mask*/);
      cute::cp_async_fence();
      preprocess_Q();
      __syncthreads(); // Make sure all threads have read smem_q before loading
                       // V
    }
    ...
    for_each(make_int_sequence<kStages>{}, [&](auto stage) {
      ...
      if constexpr (!Share_QV_Smem || !Is_first_stage) {
        if (Is_first_stage || n_block - stage >= n_block_min) {
          load_K(n_block - stage, stage,
                 cute::bool_constant<Is_first_stage>{} /*Seqlenk_mask*/);
        }
    ...
    auto fwd_step = [&](int const n_block, auto mask_fn,
      ...
      auto load_V_next = [&] {
        if (n_block - kStages + 1 >= n_block_min) {
          load_V(n_block - kStages + 1, kStages > 1 ? smem_pipe_write : 0,
                 cute::bool_constant < Is_first_iter &&
                     kStages == 1 > {} /*Seqlenk_mask*/);
        }
        cute::cp_async_fence();
      };
      ...
      Tensor scores_scale =
          softmax.template max_get_scale</*Is_first=*/Is_first_iter, Check_inf>(
              tSrS);
      ...
      flash::gemm_rs_sm80(tOrO, tOrP, tOrV,
    ...
    --n_block;
    if constexpr (Is_causal || Is_local) { // Separate iterations with causal or
                                           // local masking
...
#pragma unroll 1
  ...
  CUTLASS_DEVICE bool
  store_kv_new(Params const &params, int const thread_idx,
    ...
    Tensor mKnew = make_tensor(make_gmem_ptr(params.ptr_K_new),
                               params.shape_K_new, params.stride_K_new)(
        _, _, bidh_kv, !is_varlen_k_new ? bidb : 0);
    Tensor mVnew = make_tensor(make_gmem_ptr(params.ptr_V_new),
                               params.shape_K_new, params.stride_V_new)(
        _, _, bidh_kv, !is_varlen_k_new ? bidb : 0);
    ...
    Tensor mK =
        make_tensor(make_gmem_ptr(params.ptr_K), params.shape_K,
                    params.stride_K)(_, _, bidh_kv, !is_varlen_k ? bidb_kv : 0);
    Tensor mV =
        make_tensor(make_gmem_ptr(params.ptr_V), params.shape_K,
                    params.stride_V)(_, _, bidh_kv, !is_varlen_k ? bidb_kv : 0);

    Tensor gKnew = local_tile(
        domain_offset(make_coord(seqlen_info.offset_k_new, _0{}), mKnew),
        select<1, 2>(TileShape_MNK{}), make_coord(_, _0{})); // (N, K, _)
    Tensor gVnew = local_tile(
        domain_offset(make_coord(seqlen_info.offset_k_new, _0{}), mVnew),
        select<1, 2>(TileShape_MNK{}), make_coord(_, _0{})); // (N, K, _)
    ...
    static_assert(
        std::is_same_v<GmemLayoutAtomAppend, typename Rotary_t::LayoutAtom>);
    static_assert(
        !PagedKV ||
        std::is_same_v<GmemLayoutAtomAppend,
                       typename PagedKVManager_t::GmemLayoutAtomKVCpAsync>);
    GmemTiledCopyQKV gmem_tiled_copy_kv_g2s;
    auto gmem_thr_copy_kv_g2s =
        gmem_tiled_copy_kv_g2s.get_thread_slice(thread_idx);
    auto gmem_thr0_copy_kv_g2s = gmem_tiled_copy_kv_g2s.get_thread_slice(
        _0{}); // Only for index calculation
    GmemTiledCopyAppendKV gmem_tiled_copy_kv_s2g;
    auto gmem_thr_copy_kv_s2g =
        gmem_tiled_copy_kv_s2g.get_thread_slice(thread_idx);
    auto gmem_thr0_copy_kv_s2g = gmem_tiled_copy_kv_s2g.get_thread_slice(
        _0{}); // Only for index calculation
    Tensor tKgKnew = gmem_thr_copy_kv_g2s.partition_S(gKnew);
    Tensor tKsKg2s = gmem_thr_copy_kv_g2s.partition_S(sK);
    Tensor tKsKs2g =
        gmem_thr_copy_kv_s2g.partition_S(sK); // ((Atom,AtomNum),ATOM_M,ATOM_N)
    Tensor tKgK = gmem_thr_copy_kv_s2g.partition_D(gK);
    Tensor tVgVnew = gmem_thr_copy_kv_g2s.partition_S(
        gVnew); // ((Atom,AtomNum),ATOM_M,ATOM_N)
    Tensor tVsVg2s =
        gmem_thr_copy_kv_g2s.partition_S(sV); // ((Atom,AtomNum),ATOM_M,ATOM_N)
    Tensor tVsVs2g =
        gmem_thr_copy_kv_s2g.partition_S(sV); // ((Atom,AtomNum),ATOM_M,ATOM_N)
...
#pragma unroll
    for (int k = 0; k < size(tKpKg2s); ++k) {
      tKpKg2s(k) = get<1>(tKcKg2s(_0{}, _0{}, k)) < get<1>(params.shape_K);
    }
#pragma unroll
    for (int k = 0; k < size(tKpKs2g); ++k) {
      tKpKs2g(k) = get<1>(tKcKs2g(_0{}, _0{}, k)) < get<1>(params.shape_K);
    }

    auto load_K_new = [&](int const n_block, int const smem_pipe_write,
                          auto need_seqlenk_masking_type) {
      static constexpr bool Seqlenk_mask =
          decltype(need_seqlenk_masking_type)::value;
      static constexpr bool EvenN =
      ...
      int const seqlenk_row_limit =
          -int(get<0>(tKcKg2s(_0{}, _0{}, _0{}))) +
          (EvenN ? seqlen_k_new - n_block * kBlockN
                 : (!Seqlenk_mask
                        ? kBlockN
      ...
      flash::copy</*Is_even_MN=*/!Seqlenk_mask && EvenN, /*Is_even_K=*/false,
                  /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true>(
          gmem_tiled_copy_kv_g2s, tKgKnew(_, _, _, n_block), tKsK_cur, t0KcKg2s,
          tKpKg2s, seqlenk_row_limit);
    };

    auto load_V_new = [&](int const n_block, int const smem_pipe_write,
                          auto need_seqlenk_masking_type) {
      static constexpr bool Seqlenk_mask =
          decltype(need_seqlenk_masking_type)::value;
      static constexpr bool EvenN =
      ...
      flash::copy</*Is_even_MN=*/!Seqlenk_mask && EvenN, /*Is_even_K=*/false,
                  /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true>(
          gmem_tiled_copy_kv_g2s, tVgVnew(_, _, _, n_block), tVsV_cur, t0KcKg2s,
          tKpKg2s, seqlenk_row_limit);
    };
      ...
      if (get<1>(params.shape_rotary) <= 0) {
        ...
        if constexpr (!PagedKV) {
          ...
          flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
              ...
              gmem_tiled_copy_kv_s2g, tKsK_cur, tKgK_cur, tKcKs2g, tKpKs2g,
              std::min(seqlen_k_new - n_block * kBlockN, kBlockN));
        } else {
        ...
        auto tPrKPtr = cute::conditional_return<PagedKV>(
            paged_kv_manager.compute_K_ptr(), nullptr);
        if (params.is_rotary_interleaved) {
          auto [tRrCos, tRrSin] =
              rotary.template load_cos_sin<true /*kInterleaved*/>(n_block);
          rotary.template apply_K_interleaved<PagedKV>(
              sK(_, _, smem_pipe_read), gK_cur, tKpKs2g, tRrCos, tRrSin,
              tPrKPtr, n_block);
        } else {
          auto [tRrCosCont, tRrSinCont] =
              rotary.template load_cos_sin<false /*kInterleaved*/>(n_block);
          rotary.template apply_K_contiguous<PagedKV>(
              sK(_, _, smem_pipe_read), gK_cur, tKpKs2g, tRrCosCont, tRrSinCont,
              tPrKPtr, n_block, get<1>(params.shape_K));
        }
      }
    };
    ...
    auto store_V = [&](int const n_block, int const smem_pipe_read) {
      ...
      if constexpr (!PagedKV) {
        ...
        flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                    /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
            gmem_tiled_copy_kv_s2g, tVsV_cur, tVgV_cur, tKcKs2g, tKpKs2g,
            n_limit);
      } else {
    ...
    for_each(make_int_sequence<kStages>{}, [&](auto stage) {
      ...
      if (Is_first_stage || n_block - stage >= n_block_new_min) {
        load_K_new(n_block - stage, stage,
                   cute::bool_constant<Is_first_stage>{} /*Seqlenk_mask*/);
      }
      cute::cp_async_fence();
      // If persistent, need to sync to make sure all threads have finished with
      // smem_o before writing to smem_v
      if constexpr (Is_first_stage) {
        __syncthreads();
      }
      if constexpr (!Is_last_stage) {
        if (Is_first_stage || n_block - stage >= n_block_new_min) {
          load_V_new(n_block - stage, stage,
                     cute::bool_constant<Is_first_stage>{} /*Seqlenk_mask*/);
        }
        cute::cp_async_fence();
      }
...
#pragma unroll 1
    for (; n_block >= n_block_new_min; --n_block) {
      ...
      if (n_block - kStages + 1 >= n_block_new_min) {
        load_V_new(n_block - kStages + 1, kStages > 1 ? smem_pipe_write : 0,
                   cute::bool_constant<kStages == 1>{} /*Seqlenk_mask*/);
      }
      cute::cp_async_fence();
      smem_pipe_write = smem_pipe_write < kStages - 1 ? smem_pipe_write + 1 : 0;
      flash::cp_async_wait<kStages * 2 - 2>();
      ...
      if (n_block - kStages >= n_block_new_min) {
        load_K_new(n_block - kStages, kStages > 1 ? smem_pipe_write : 0,
                   cute::false_type{} /*Seqlenk_mask*/);
      }
      cute::cp_async_fence();
    }
