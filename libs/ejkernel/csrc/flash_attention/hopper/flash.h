...
struct Flash_fwd_params : public Qkv_params {
  ...
  int b_k; // When having KV cache and with cache_batch_idx, K & V might have
           // larger batch size than Q
  int dv,
      dv_rounded; // For the case where V headdim is different from Q/K headdim
...
void prepare_varlen_num_blocks(Flash_fwd_params &params, cudaStream_t stream,
                               bool packgqa, int blockM, int blockN,
                               bool enable_pdl);
