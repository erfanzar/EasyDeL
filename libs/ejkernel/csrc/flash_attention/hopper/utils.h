...
template <typename Kernel> struct enable_sm80_to_sm89 : Kernel {
  template <typename... Args> CUTLASS_DEVICE void operator()(Args &&...args) {
...
int div_floor(cutlass::FastDivmod const &divmod, int dividend) {
  ...
  // Maybe the compiler will turn the -1 - * into bit negation operation, I
  // haven't checked.
  return dividend >= 0 ? divmod.divide(dividend)
                       : -1 - divmod.divide(-1 - dividend);
}
...
CUTLASS_DEVICE auto convert_type_unsafe(Tensor<Engine, Layout> const &tensor) {
  ...
  // Unsafe because we're returning a tensor with memory allocated on the stack.
  // If the compiler does not inline this function, then the memory might not be
  // valid.
}
...
CUTLASS_DEVICE auto mma_partition_fragment_AB(Mma const &mma,
...
template <bool zero_init = false, int wg_wait = 0, bool SwapAB = false,
          int M_slice = -1, typename Tensor0, typename Tensor1,
          typename Tensor2, typename TiledMma>
CUTLASS_DEVICE void gemm(TiledMma &tiled_mma, Tensor0 const &tCrA,
  ...
  if constexpr (M_slice >= 0) {
    ...
    if constexpr (!SwapAB) {
      Tensor tCrA_slice =
          cute::logical_divide(tCrA, Shape<cute::Underscore, Int<MMA_M>>{})(
              _, make_coord(Int<M_slice>{}, _), _);
      gemm<zero_init, wg_wait, SwapAB, /*M_slice=*/-1>(tiled_mma, tCrA_slice,
                                                       tCrB, tCrC_slice);
    } else {
      Tensor tCrB_slice =
          cute::logical_divide(tCrB, Shape<cute::Underscore, Int<MMA_M>>{})(
              _, make_coord(Int<M_slice>{}, _), _);
      gemm<zero_init, wg_wait, SwapAB, /*M_slice=*/-1>(tiled_mma, tCrA,
                                                       tCrB_slice, tCrC_slice);
    }
  } else {
    ...
    for (int k_block = 0; k_block < std::min(kNumKIters, kMaxKIters);
         ++k_block) {
      if constexpr (!SwapAB) {
    ...
    // In the case of large kNumKIters, the compiler chooses to store the smem
    // addresses in registers, causing spills. This loop forces the compiler to
    // recompute the addresses.
    if constexpr (kNumKIters > kMaxKIters) {
      // This will always be zero, just a way to force the compiler to recompute
      // the smem addresses. This results in USEL instructions. There's probably
      // a better way to do this.
      int const k_offset = cutlass::canonical_warp_group_idx() < 128 ? 0 : 1;
      CUTLASS_PRAGMA_UNROLL
      for (int k_block = kMaxKIters; k_block < kNumKIters; ++k_block) {
        if constexpr (!SwapAB) {
          cute::gemm(tiled_mma, tCrA(_, _, k_block + k_offset),
                     tCrB(_, _, k_block + k_offset), tCrC);
        } else {
          cute::gemm(tiled_mma, tCrB(_, _, k_block + k_offset),
                     tCrA(_, _, k_block + k_offset), tCrC);
        }
        tiled_mma.accumulate_ = GMMA::ScaleOut::One;
      }
    }
...
CUTLASS_DEVICE void
gemm_sm80(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsA,
          Tensor4 const &tCsB, TiledMma tiled_mma, TiledCopyA smem_tiled_copy_A,
          TiledCopyB smem_tiled_copy_B, ThrCopyA smem_thr_copy_A,
          ThrCopyB smem_thr_copy_B, Hook fn) {
  if constexpr (SwapAB) {
    gemm_sm80<B_in_regs, A_in_regs>(acc, tCrB, tCrA, tCsB, tCsA, tiled_mma,
                                    smem_tiled_copy_B, smem_tiled_copy_A,
                                    smem_thr_copy_B, smem_thr_copy_A, fn);
  } else {
    ...
    if (!A_in_regs) {
      cute::copy(smem_tiled_copy_A, tCsA(_, _, _0{}),
                 tCrA_copy_view(_, _, _0{}));
    }
    if (!B_in_regs) {
      cute::copy(smem_tiled_copy_B, tCsB(_, _, _0{}),
...
template <typename Tensor0, typename Tensor1, typename Tensor2,
          ...
          typename ThrCopy>
CUTLASS_DEVICE void gemm_rs_sm80(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB,
...
template <bool zero_init = false, typename Atom, typename TA, typename TB,
          typename TC>
CUTLASS_DEVICE void gemm_sm100(Atom &atom, TA const &tA, TB const &tB,
                               TC &&tC) {
...
template <class a_type, class b_type, class c_type, int M, int N,
          ...
          UMMA::ScaleIn b_neg, class... TAs, class... TMs>
CUTE_HOST_DEVICE constexpr auto to_tiled_mma_sm100_ts(
    TiledMMA<MMA_Atom<MMA_Traits<SM100_MMA_F8F6F4_SS, a_type, b_type, c_type,
                                 cute::C<M>, cute::C<N>,
  ...
  return TiledMMA<MMA_Atom<MMA_Traits<SM100_MMA_F8F6F4_TS<
                               a_type, b_type, c_type, M, N, a_major, b_major,
...
template <class a_type, class b_type, class c_type, int M, int N,
          ...
          UMMA::ScaleIn b_neg, class... TAs, class... TMs>
CUTE_HOST_DEVICE constexpr auto to_tiled_mma_sm100_ts(
    TiledMMA<MMA_Atom<SM100_MMA_F16BF16_SS<a_type, b_type, c_type, M, N,
                                           a_major, b_major, a_neg, b_neg>,
                      TAs...>,
             TMs...>) {
  return TiledMMA<MMA_Atom<SM100_MMA_F16BF16_TS<a_type, b_type, c_type, M, N,
                                                a_major, b_major, a_neg, b_neg,
                                                UMMA::Saturate::False>,
                           TAs...>,
                  TMs...>{};
...
template <bool Is_even_MN = true, bool Is_even_K = true,
          bool Clear_OOB_MN = false, bool Clear_OOB_K = true, class CopyAtom,
          class TV, class Tiler, typename Engine0, typename Layout0,
...
#pragma unroll
        for (int k = 0; k < size<2>(S); ++k) {
          if constexpr (Is_even_K || !Clear_OOB_K) {
            ...
            // If copy traits can be transformed with a predicate value, do it,
            // otherwise branch here
            if constexpr (has_with_bool) {
              cute::copy(copy_atom.with(predicate_K(k)), S(_, m, k),
                         D(_, m, k));
            } else {
              if (predicate_K(k)) {
                cute::copy(copy_atom, S(_, m, k), D(_, m, k));
...
CUTLASS_DEVICE void apply_softcap(Tensor<Engine, Layout> &tensor,
                                  float const softcap) {
#pragma unroll
...
template <class T> CUTE_DEVICE T warp_prefix_sum(T val) {
...
template <class T> CUTE_DEVICE T warp_uniform(T a) {
