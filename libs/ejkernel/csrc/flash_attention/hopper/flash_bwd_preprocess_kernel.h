...
template <class TileShape_MK_, class Element, class ElementAccum,
          class ArchTag_, bool Clear_dQaccum, bool Varlen>
...
public:
  ...
  static_assert((kBlockM * kHeadDim / kGmemElemsPerLoadAccum) %
                        MaxThreadsPerBlock ==
                    0,
                "MaxThreadsPerBlock must divide kBlockM * kHeadDim / "
  ...
  void operator()(Params const &params, [[maybe_unused]] char *smem_buf) {
    ...
    Tensor gLSE =
        local_tile(cute::domain_offset(make_coord(seqlen_info.offset), mLSE),
...
#pragma unroll
    ...
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/true, /*Clearn_OOB_K=*/true>(
        gmem_tiled_copy_O, tOgO, tOrO, tOcO, tOpO,
        seqlen_o - m_block * kBlockM);
    flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false,
                /*Clear_OOB_MN=*/true, /*Clearn_OOB_K=*/true>(
        gmem_tiled_copy_O, tOgdO, tOrdO, tOcO, tOpO,
        seqlen_o - m_block * kBlockM);
    // if (threadIdx.x == 222) { printf("bidx = %d, bidy = %d, bidz = %d,
    // seqlen_o = %d, m_block = %d, seqlen_o - m_block * kBlockM = %d, tOgO addr
...
#pragma unroll
    ...
    if constexpr (Clear_dQaccum) {
      ...
      Tensor gdQaccum = local_tile(
          cute::domain_offset(make_coord(seqlen_info.offset_padded * kHeadDim),
                              mdQaccum),
          Shape<Int<kBlockM * kHeadDim>>{}, make_coord(m_block));
