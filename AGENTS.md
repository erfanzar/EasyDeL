...
commands, architecture, and the agent playbook in one place. `CLAUDE.md` is a
symlink to this file (same pattern as `.claude` → `.agents`), so Claude Code
and AGENTS.md-reading tools load identical content. Everything here is
...
[.agents/repo-map.yaml](.agents/repo-map.yaml). Non-trivial work starts from
`.agents/skills/run-research/SKILL.md` and layers a domain skill on top.
...
- Specialized subagents: `.agents/agents/` (roster and flows below).
