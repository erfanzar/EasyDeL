...
class FlaxQwen2Attention(BaseJAXAttentionModule):
    config: Qwen2Config
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype = jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]] = None
    ...
    def __call__(
        ...
        query_state = with_sharding_constraint(query_state, PartitionSpec(("dp", "fsdp"), "sp" if query_state.shape[1] != 1 else None, "tp"))
...
class FlaxQwen2PreTrainedModel(EasyDelFlaxPretrainedModel):
